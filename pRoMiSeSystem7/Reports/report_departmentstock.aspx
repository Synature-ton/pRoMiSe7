...
<html>
<head>
<title>Department Stock Reports</title>
...
Sub Page_Load()
	If User.Identity.IsAuthenticated  AND Session("DepartmentStock") Then
	...
	'Try	
		...
		LangText0.Text = "รายงานต้นทุนครัว"
		...
		If IsNumeric(Request.Form("DepartmentCode")) Then
			DepartmentCode = Request.Form("DepartmentCode")
		Else If IsNumeric(Request.QueryString("DepartmentCode"))
			DepartmentCode = Request.QueryString("DepartmentCode")
		End If
		...
		Dim DepartmentData As DataTable = objDB.List("select * from Department where Deleted=0 Order By DepartmentID", objCnn)
		If DepartmentData.Rows.Count > 0 Then

			outputString = "<select name=""DepartmentCode"" style=""width:300px;"">"
			If DepartmentData.Rows.Count > 1 Then
				If Not Page.IsPostBack Then 
					FormSelected = "selected"
				ElseIf DepartmentCode = "" Then
					FormSelected = "selected"
				Else
					FormSelected = ""
				End If
				outputString += "<option value=""" & "" & """ " & FormSelected & ">" & "--- All ---"
			End If

			For i = 0 to DepartmentData.Rows.Count - 1
				If DepartmentCode = DepartmentData.Rows(i)("DepartmentCode") Then
					FormSelected = "selected"
				Else
					FormSelected = ""
				End If
				outputString += "<option value=""" & DepartmentData.Rows(i)("DepartmentCode") & """ " & FormSelected & ">" & DepartmentData.Rows(i)("DepartmentName")
			Next
			outputString += "</select>"
		...
		If ShopData.Rows.Count > 0 Then
			...
			outputString = "<select name=""ShopID"" style=""width:300px;"">"
...
Sub DoSearch(Source As Object, E As EventArgs)
	...
	If FoundError = False Then
		...
		Dim SaleDay As Integer
		Dim TotalSale As Double
		Dim TableNameString As String

		Dim dtTable As DataTable = DepartmentStockReport(TotalSale,SaleDay,TableNameString,Request.Form("ShopID"),Request.Form("DepartmentCode"), StartDate, EndDate, SelMonth, SelYear, Session("LangID"), "", objCnn)
	
		Dim HeaderString As String = "<tr>"
		HeaderString += "<td align=""center"" class=""smallTdHeader"" bgColor=""" + GlobalParam.AdminBGColor + """>" + "No" + "</td>"
		HeaderString += "<td align=""center"" class=""smallTdHeader"" bgColor=""" + GlobalParam.AdminBGColor + """>" + "รหัสแผนก" + "</td>"
		HeaderString += "<td align=""center"" class=""smallTdHeader"" bgColor=""" + GlobalParam.AdminBGColor + """>" + "แผนก" + "</td>"
		HeaderString += "<td align=""center"" class=""smallTdHeader"" bgColor=""" + GlobalParam.AdminBGColor + """>" + "มูลค่า" + "</td>"
		HeaderString += "<td align=""center"" class=""smallTdHeader"" bgColor=""" + GlobalParam.AdminBGColor + """>" + "คิด%" + "</td>"
		HeaderString += "<td align=""center"" class=""smallTdHeader"" bgColor=""" + GlobalParam.AdminBGColor + """>" + "ต่อวัน" + "</td>"
		HeaderString += "</tr>"
		headerTextString.InnerHtml = HeaderString
		
		Dim outputString As StringBuilder = New StringBuilder
		Dim grandTotalCost As Double = 0
		Dim grandTotalP As Double = 0
		...
		For i = 0 to dtTable.Rows.Count - 1
			...
			outputString = outputString.Append("<td class=""smallText"" align=""left"">" + dtTable.Rows(i)("DepartmentCode") + "</td>")
			outputString = outputString.Append("<td class=""smallText"" align=""left"">" + dtTable.Rows(i)("DepartmentName") + "</td>")
			outputString = outputString.Append("<td class=""smallText"" align=""right"">" + CDbl(dtTable.Rows(i)("TotalCost")).ToString(FormatObject.CurrencyFormat,ci) + "</td>")
			If TotalSale <> 0 Then
				outputString = outputString.Append("<td class=""smallText"" align=""right"">" + CDbl(dtTable.Rows(i)("TotalCost")/TotalSale).ToString(FormatObject.PercentFormat,ci) + "</td>")
				grandTotalP += Math.Round(dtTable.Rows(i)("TotalCost")/TotalSale,2)
			Else
				outputString = outputString.Append("<td class=""smallText"" align=""right"">" + "-" + "</td>")
			End If
			If SaleDay <> 0 Then
				outputString = outputString.Append("<td class=""smallText"" align=""right"">" + CDbl(dtTable.Rows(i)("TotalCost")/SaleDay).ToString(FormatObject.CurrencyFormat,ci) + "</td>")
			Else
				outputString = outputString.Append("<td class=""smallText"" align=""right"">" + "-" + "</td>")
			End If
			outputString = outputString.Append("</tr>")
			grandTotalCost += dtTable.Rows(i)("TotalCost")
			
		Next
		outputString = outputString.Append("<tr bgColor=""" + GlobalParam.GrayBGColor + """>")
		outputString = outputString.Append("<td class=""smallText"" colspan=""3"" align=""right"">" + "สรุุป" + "</td>")
		outputString = outputString.Append("<td class=""smallText"" align=""right"">" + CDbl(grandTotalCost).ToString(FormatObject.CurrencyFormat,ci) + "</td>")
		outputString = outputString.Append("<td class=""smallText"" align=""right"">" + CDbl(grandTotalP).ToString(FormatObject.PercentFormat,ci) + "</td>")
		outputString = outputString.Append("<td class=""smallText"" align=""right"">" + CDbl(TotalSale).ToString(FormatObject.CurrencyFormat,ci) + "</td>")
		outputString = outputString.Append("</tr>")
...
Public Function DepartmentStockReport(ByRef TotalSale As Double,ByRef SaleDay As Integer,ByVal TableNameString As String, ByVal ShopID As Integer, ByVal DepartmentCode As String, ByVal StartDate As String, ByVal EndDate As String, ByVal SelMonth As Integer, ByVal SelYear As Integer, ByVal LangID As Integer, ByVal OrderByString As String, ByVal objCnn As MySqlConnection) As DataTable

        Dim sqlStatement, AdditionalQuery, WString As String


        If ShopID > 0 Then
            AdditionalQuery += " AND a.ProductLevelID=" + ShopID.ToString
			WString += " AND ProductLevelID=" + ShopID.ToString
        End If
		
		If DepartmentCode <> "" Then
			 AdditionalQuery += " AND a.InvoiceRef='" + Replace(DepartmentCode,"'","''") + "'"
		End If

        sqlStatement = " select a.InvoiceRef As DisplayCode,c.DepartmentCode,c.DepartmentName,c.DepartmentID,SUM(b.ProductNetPrice) As TotalCost from Document a left outer join DocDetail b ON a.DocumentID=b.DocumentID AND a.ShopID=b.ShopID left outer join Department c ON a.InvoiceRef=c.DepartmentCode where a.DocumentStatus=2 AND a.DocumentTypeID=6 AND a.DocumentDate >= " + StartDate + " AND a.DocumentDate < " + EndDate + AdditionalQuery + " group by a.InvoiceRef,c.DepartmentCode,c.DepartmentName,c.DepartmentID order by c.DepartmentID" ' + OrderByString
		
		'errorMsg.InnerHtml = sqlStatement
		
		Dim getDay As DataTable = objDB.List("select count(*) As TotalDay from SessionEndDayDetail where IsEndDay=1 AND SessionDate >= " + StartDate + " AND SessionDate < " + EndDate + WString, objCnn)
		
		If Not IsDBNull(getDay.Rows(0)("TotalDay")) Then
			SaleDay = getDay.Rows(0)("TotalDay")
		Else
			SaleDay = 0
		End If
		
		Dim getSale As DataTable = objDB.List("select SUM(ReceiptSalePrice) As TotalSale from ordertransaction where TransactionStatusID=2 AND ReceiptID>0 AND SaleDate >= " + StartDate + " AND SaleDate < " + EndDate, objCnn)
		
		If Not IsDBNull(getSale.Rows(0)("TotalSale")) Then
			TotalSale = getSale.Rows(0)("TotalSale")
		Else
			TotalSale = 0
		End If
...
Sub ExportData(Source As Object, E As EventArgs)
	...
	Dim FileName As String = "DepartmentCostData_" & DateTime.Now.ToString("yyyyMMdd_HHmmss", InvC) & ".xls"
