...
<tr bgcolor="eeeeee">
	...
	<td colspan="2" background="../images/headerbg2000.jpg">
		<div class="noprint"><table width="100%"><tr><td align="left" width="70%"><b class="headerText"><asp:Label ID="LangText0" Text="Daily Report" runat="server" /></b></td><td align="right"><asp:Label ID="LangList" runat="server" /></td></tr></table></div>
	</td>
...
Sub Page_Load()
	If User.Identity.IsAuthenticated  AND Session("ReportDailySummary") Then
...
Sub DoSearch(Source As Object, E As EventArgs)
	...
	If FoundError = False Then
		...
		Dim dtTable As DataTable = DailyReport(ShowGroup.Checked,outputString, outString2, outString3, StartDate, EndDate, Request.Form("ShopID"),0,0, "pg.ProductGroupCode,pd.ProductDeptCode", GlobalParam.AdminBGColor, Session("LangID"), BeginDay, NoDay, objCnn)
		...
		End If
		ResultSearchText.InnerHtml = "Daily Report of " + ShopDisplay + " (" + ReportDate + ")"
...
End Sub	
    ...
    Public Function DailyReport(ByVal ShowGroup As Boolean, ByRef ResultString As String, ByRef ResultString2 As String, ByRef ResultString3 As String, ByVal StartDate As String, ByVal EndDate As String, ByVal ShopID As Integer, ByVal TransactionID As Integer, ByVal ComputerID As Integer, ByVal OrderBy As String, ByVal AdminBGColor As String, ByVal LangID As Integer, ByVal BeginDay As String, ByVal NoDay As Integer, ByVal objCnn As MySqlConnection) As DataTable
        ...
        sqlStatement = "select IF(pd.ProductDeptCode is NULL,'-',pd.ProductDeptCode) As ProductDeptCode,IF(pd.ProductDeptName is NULL,'Other Category',pd.ProductDeptName) As ProductDeptName,IF(pg.ProductGroupCode is NULL,'-',pg.ProductGroupCode) As ProductGroupCode,IF(pg.ProductGroupName is NULL,'Other Category',pg.ProductGroupName) As ProductGroupName, SUM(b.Amount) As TotalQty, SUM(IF(b.ProductSetType>=0,c.TotalRetailPrice,0)) AS TotalRetailPrice, SUM(c.SalePrice) As TotalSalePrice, SUM(IF(b.ProductSetType>=0,c.TotalRetailPrice-c.SalePrice,0)) As TotalDiscount from OrderTransaction a left outer join OrderDetail b ON a.TransactionID=b.TransactionID AND a.ComputerID=b.ComputerID left outer join OrderDiscountDetail c ON b.OrderDetailID=c.OrderDetailID AND b.TransactionID=c.TransactionID AND b.ComputerID=c.ComputerID left outer join Products p ON b.ProductID=p.ProductID left outer join ProductDept pd ON p.ProductDeptID=pd.ProductDeptID left outer join ProductGroup pg ON pd.ProductGroupID=pg.ProductGroupID where a.ReceiptID > 0 AND b.ProductSetType NOT IN (-1,-3) AND a.TransactionStatusID=2 AND a.DocType=8 AND b.OrderStatusID IN (1,2,5) " + AdditionalQuery + " group by pd.ProductDeptCode,pd.ProductDeptName,pg.ProductGroupCode,pg.ProductGroupName order by " + OrderBy
		...
		sqlStatement = "select *,EDC_BBL_Visa+EDC_BBL_Amex+EDC_BBL_Master+EDC_BBL_Other+EDC_Kasikorn_Visa+EDC_Kasikorn_Master+EDC_Kasikorn_AMex+EDC_Kasikorn_Other As CCAmount from SessionOotoyaDailyDetail where 0=0 " + SessionString 
		Dim EndDayData As DataTable = objDB.List(sqlStatement, objCnn)
		...
		sqlStatement = "select b.CreditCardType,c.CreditCardType As CreditCardTypeName,SUM(b.Amount) As PayAmount from ordertransaction a left outer join paydetail b ON a.TransactionID=b.TransactionID AND a.ComputerID=b.ComputerID left outer join creditcardtype c ON b.CreditCardType=c.CCTypeID where a.ReceiptID>0 AND a.DocType=8 AND a.TransactionStatusID=2 AND b.PayTypeID=2 " + AdditionalQuery + " Group By b.CreditCardType,c.CreditCardType Order By b.CreditCardType"
		Dim CCTypeData As DataTable = objDB.List(sqlStatement, objCnn)  
		...
		sqlStatement = "select TRIM(REPLACE(REPLACE(ProductName,'(T)',''),'(S)','')) As ProductName,SUM(SalePrice) As SalePrice,SUM(Amount) As TotalQty from Summary_ProductReport a where TransactionStatusID=2 AND ProductSetType NOT IN (-1,-3,14,16) AND a.DocType=8 AND a.ProductDeptCode NOT IN ('P1','106','107','108','109','110') AND ProductCode NOT IN ('109027','109028') " + AdditionalQuery + " group by TRIM(REPLACE(REPLACE(ProductName,'(T)',''),'(S)','')) order by SUM(Amount) DESC, SUM(SalePrice) DESC LIMIT 0,3"
		Dim TopMenu As DataTable = objDB.List(sqlStatement, objCnn)
		
		sqlStatement = "select a.*,CONCAT(s1.StaffFirstName,' ',s1.StaffLastName) As OpenStaff,CONCAT(s2.StaffFirstName,' ',s2.StaffLastName) As CloseStaff from Session a left outer join Staffs s1 ON a.OpenStaffID=s1.StaffID left outer join Staffs s2 ON a.CloseStaffID=s2.StaffID where 0=0 " + SessionString + " order by SessionID"
		...
		If VoidData.Rows.Count > 0 Then
			...
			VoidAmount = VoidData.Rows(0)("SalePrice") + VoidData.Rows(0)("ServiceCharge") + VoidData.Rows(0)("ServiceChargeVAT") + VoidData.Rows(0)("OtherIncome") + VoidData.Rows(0)("OtherIncomeVAT")
		End If
		...
		Dim BBLVisa As Double = 0
		Dim BBLMaster As Double = 0
		Dim BBLAmex As Double = 0
		Dim BBLOther As Double = 0
		Dim KBankVisa As Double = 0
		Dim KBankMaster As Double = 0
		Dim KBankAmex As Double = 0
		Dim KBankOther As Double = 0
		...
		If EndDayData.Rows.Count > 0 Then
			
			SummaryTotal += EndDayData.Rows(0)("CashAmount") + EndDayData.Rows(0)("CCAmount")
			TipAmount = EndDayData.Rows(0)("TipAmount")
			TipPerson = EndDayData.Rows(0)("TipPerson")
			BBLVisa = EndDayData.Rows(0)("EDC_BBL_Visa")
			BBLMaster = EndDayData.Rows(0)("EDC_BBL_Master")
			BBLAmex = EndDayData.Rows(0)("EDC_BBL_Amex")
			BBLOther = EndDayData.Rows(0)("EDC_BBL_Other")
			KBankVisa = EndDayData.Rows(0)("EDC_Kasikorn_Visa")
			KBankMaster = EndDayData.Rows(0)("EDC_Kasikorn_Master")
			KBankAmex = EndDayData.Rows(0)("EDC_Kasikorn_Amex")
			KBankOther = EndDayData.Rows(0)("EDC_Kasikorn_Other")
			RemarkText = EndDayData.Rows(0)("Remark")
			CashInputAmount = EndDayData.Rows(0)("CashAmount")
			CCInputAmount = EndDayData.Rows(0)("CCAmount")
			CashAmount = EndDayData.Rows(0)("CashAmount")
		End If
		outputString = outputString.Append("<tr bgColor=""" + bgColor + """>")
		outputString = outputString.Append("<td align=""left"" class=""" + TextClass + """>" + "Cash (เงินสด)" + "</td>")
		outputString = outputString.Append("<td align=""right"" class=""" + TextClass + """>" + CDbl(CashInputAmount).ToString(FormatObject.CurrencyFormat, ci) + "</td>")
		outputString = outputString.Append("</tr>")
		
		outputString = outputString.Append("<tr bgColor=""" + bgColor + """>")
		outputString = outputString.Append("<td align=""left"" class=""" + TextClass + """>" + "Credit (การ์ด)" + "</td>")
		outputString = outputString.Append("<td align=""right"" class=""" + TextClass + """>" + CDbl(CCInputAmount).ToString(FormatObject.CurrencyFormat, ci) + "</td>")
		outputString = outputString.Append("</tr>")
		For i = 0 To PaymentData.Rows.Count - 1
			If PaymentData.Rows(i)("PayTypeID") <> 1 AND PaymentData.Rows(i)("PayTypeID") <> 2 Then
				outputString = outputString.Append("<tr bgColor=""" + bgColor + """>")
				outputString = outputString.Append("<td align=""left"" class=""" + TextClass + """>" + PaymentData.Rows(i)("PayTypeName") + "</td>")
				outputString = outputString.Append("<td align=""right"" class=""" + TextClass + """>" + CDbl(PaymentData.Rows(i)("TotalPay")).ToString(FormatObject.CurrencyFormat, ci) + "</td>")
				outputString = outputString.Append("</tr>")
				SummaryTotal += PaymentData.Rows(i)("TotalPay")
			ElseIf PaymentData.Rows(i)("PayTypeID") = 1 Then
				CashSystem = PaymentData.Rows(i)("TotalPay")
			End If
		Next
		
		outputString = outputString.Append("<tr bgColor=""#ebebeb"">")
		outputString = outputString.Append("<td align=""left"" class=""" + TextClass + """>" + "Total" + "</td>")
		outputString = outputString.Append("<td align=""right"" class=""" + TextClass + """>" + CDbl(SummaryTotal).ToString(FormatObject.CurrencyFormat, ci) + "</td>")
		outputString = outputString.Append("</tr>")
		
		outputString = outputString.Append("<tr bgColor=""" + bgColor + """>")
		outputString = outputString.Append("<td align=""left"" class=""" + TextClass + """>" + "Cash Change (เงินทอน)" + "</td>")
		If EndDaySession.Rows.Count > 0 Then
			outputString = outputString.Append("<td align=""right"" class=""" + TextClass + """>" + CDbl(EndDaySession.Rows(0)("OpenSessionAmount")).ToString(FormatObject.CurrencyFormat, ci) + "</td>")
			CashChange = EndDaySession.Rows(0)("OpenSessionAmount")
			SummaryTotal -= EndDaySession.Rows(0)("OpenSessionAmount")
		Else
		...
		outputString = outputString.Append("<tr bgColor=""" + bgColor + """>")
		outputString = outputString.Append("<td align=""left"" class=""" + TextClass + """>" + "Actual Sales (ยอดขายจริง)" + "</td>")
		outputString = outputString.Append("<td align=""right"" class=""" + TextClass + """>" + CDbl(SummaryTotal).ToString(FormatObject.CurrencyFormat, ci) + "</td>")
		outputString = outputString.Append("</tr>")
		
		outputString = outputString.Append("<tr bgColor=""" + bgColor + """>")
		outputString = outputString.Append("<td align=""left"" class=""" + TextClass + """>" + "Total Sales (ยอดขายทั้งหมด)" + "</td>")
		If SaleData.Rows.Count > 0 Then
			...
			ShortOver = SummaryTotal - TodaySale
		...
		Else
			outputString = outputString.Append("<td align=""right"" class=""" + TextClass + """>" + "-" + "</td>")
			ShortOver = SummaryTotal
			TodaySale = 0
		End If
		outputString = outputString.Append("</tr>")
		
		outputString = outputString.Append("<tr bgColor=""" + bgColor + """>")
		outputString = outputString.Append("<td align=""left"" class=""" + TextClass + """>" + "Over/Short (เงินเกิน/ขาด)" + "</td>")
		outputString = outputString.Append("<td align=""right"" class=""" + TextClass + """>" + CDbl(ShortOver).ToString(FormatObject.CurrencyFormat, ci) + "</td>")
		outputString = outputString.Append("</tr>")
		
		outputString = outputString.Append("<tr bgColor=""#ebebeb"">")
		outputString = outputString.Append("<td align=""left"" class=""" + TextClass + """>" + "Bank Deposit (เข้าธนาคาร)" + "</td>")
		outputString = outputString.Append("<td align=""right"" class=""" + TextClass + """>" + CDbl(CashAmount-CashChange).ToString(FormatObject.CurrencyFormat, ci) + "</td>")
		outputString = outputString.Append("</tr>")
		
		outputString = outputString.Append("<tr>")
		outputString = outputString.Append("<td align=""center"" class=""smallTdHeader"" bgcolor=""" + AdminBGColor + """>" + "Summary" + "</td>")
		outputString = outputString.Append("<td align=""center"" class=""smallTdHeader"" bgcolor=""" + AdminBGColor + """>" + "Amount" + "</td>")
		outputString = outputString.Append("</tr>")
		
		outputString = outputString.Append("<tr bgColor=""" + bgColor + """>")
		outputString = outputString.Append("<td align=""left"" class=""" + TextClass + """>" + "Net Sales (ยอดขาย)" + "</td>")
		outputString = outputString.Append("<td align=""right"" class=""" + TextClass + """>" + CDbl(NetSale).ToString(FormatObject.CurrencyFormat, ci) + "</td>")
		outputString = outputString.Append("</tr>")
		
		outputString = outputString.Append("<tr bgColor=""" + bgColor + """>")
		outputString = outputString.Append("<td align=""left"" class=""" + TextClass + """>" + "Discount (ส่วนลด)" + "</td>")
		...
		outputString = outputString.Append("<tr bgColor=""#ebebeb"">")
		outputString = outputString.Append("<td align=""left"" class=""" + TextClass + """>" + "Net Sales  after Discount" + "</td>")
		...
		outputString = outputString.Append("<tr bgColor=""" + bgColor + """>")
		outputString = outputString.Append("<td align=""left"" class=""" + TextClass + """>" + "Service Charge" + "</td>")
		...
		outputString = outputString.Append("<tr bgColor=""#ebebeb"">")
		outputString = outputString.Append("<td align=""left"" class=""" + TextClass + """>" + "Today Sales (รวมรายได้)" + "</td>")
		outputString = outputString.Append("<td align=""right"" class=""" + TextClass + """>" + CDbl(TodaySale).ToString(FormatObject.CurrencyFormat, ci) + "</td>")
		outputString = outputString.Append("</tr>")
		
		outputString = outputString.Append("<tr bgColor=""" + bgColor + """>")
		outputString = outputString.Append("<td align=""left"" class=""" + TextClass + """>" + "VAT 7%" + "</td>")
		outputString = outputString.Append("<td align=""right"" class=""" + TextClass + """>" + CDbl(TodayVAT).ToString(FormatObject.CurrencyFormat, ci) + "</td>")
		outputString = outputString.Append("</tr>")
		
		outputString = outputString.Append("<tr bgColor=""" + bgColor + """>")
		outputString = outputString.Append("<td align=""left"" class=""" + TextClass + """>" + "Sales Before VAT" + "</td>")
		...
		outputString = outputString.Append("<tr bgColor=""" + bgColor + """>")
		outputString = outputString.Append("<td align=""left"" class=""" + TextClass + """>" + "Pax (Customers)" + "</td>")
		outputString = outputString.Append("<td align=""right"" class=""" + TextClass + """>" + CDbl(TodayCustomer).ToString(FormatObject.QtyFormat, ci) + "</td>")
		outputString = outputString.Append("</tr>")
		
		outputString = outputString.Append("<tr bgColor=""" + bgColor + """>")
		outputString = outputString.Append("<td align=""left"" class=""" + TextClass + """>" + "Price/Customer" + "</td>")
		outputString = outputString.Append("<td align=""right"" class=""" + TextClass + """>" + CDbl(TodaySale/TodayCustomer).ToString(FormatObject.NumericFormat, ci) + "</td>")
		outputString = outputString.Append("</tr>")
		
		outputString = outputString.Append("<tr bgColor=""" + bgColor + """>")
		outputString = outputString.Append("<td align=""left"" class=""" + TextClass + """>" + "No Bills" + "</td>")
		outputString = outputString.Append("<td align=""right"" class=""" + TextClass + """>" + CDbl(TodayBill).ToString(FormatObject.QtyFormat, ci) + "</td>")
		outputString = outputString.Append("</tr>")
		
		outputString = outputString.Append("<tr bgColor=""" + bgColor + """>")
		outputString = outputString.Append("<td align=""left"" class=""" + TextClass + """>" + "Price/Bill" + "</td>")
		outputString = outputString.Append("<td align=""right"" class=""" + TextClass + """>" + CDbl(TodaySale/TodayBill).ToString(FormatObject.NumericFormat, ci) + "</td>")
		outputString = outputString.Append("</tr>")
		
		outputString = outputString.Append("<tr>")
		outputString = outputString.Append("<td colspan=""2"" align=""center"" class=""smallTdHeader"" bgcolor=""" + AdminBGColor + """>" + "No Customers (จำนวนลูกค้า)" + "</td>")
		outputString = outputString.Append("</tr>")
		
		For i = 0 To CustomerData.Rows.Count - 1
			outputString = outputString.Append("<tr bgColor=""" + bgColor + """>")
			outputString = outputString.Append("<td align=""left"" class=""" + TextClass + """>" + CustomerData.Rows(i)("QDDName") + "</td>")
			outputString = outputString.Append("<td align=""right"" class=""" + TextClass + """>" + CDbl(CustomerData.Rows(i)("TotalCustomer")).ToString(FormatObject.QtyFormat, ci) + "</td>")
			outputString = outputString.Append("</tr>")
			TotalCustomers += CustomerData.Rows(i)("TotalCustomer")
		Next
		outputString = outputString.Append("<tr bgColor=""#ebebeb"">")
		outputString = outputString.Append("<td align=""left"" class=""" + TextClass + """>" + "Total Customers" + "</td>")
		outputString = outputString.Append("<td align=""right"" class=""" + TextClass + """>" + CDbl(TotalCustomers).ToString(FormatObject.QtyFormat, ci) + "</td>")
		outputString = outputString.Append("</tr>")
		...
		outputString2 = outputString2.Append("<td colspan=""2"" align=""center"" class=""smallTdHeader"" bgcolor=""" + AdminBGColor + """>" + "ใบสรุปจากเครื่อง EDC" + "</td>")
		outputString2 = outputString2.Append("</tr>")
        
		outputString2 = outputString2.Append("<tr bgColor=""#ebebeb"">")
		outputString2 = outputString2.Append("<td colspan=""2"" align=""center"" class=""" + TextClass + """>" + "BBL" + "</td>")
		outputString2 = outputString2.Append("</tr>")
		
		outputString2 = outputString2.Append("<tr bgColor=""" + bgColor + """>")
		outputString2 = outputString2.Append("<td align=""left"" class=""" + TextClass + """>" + "Visa" + "</td>")
		outputString2 = outputString2.Append("<td width=""100"" align=""right"" class=""" + TextClass + """>" + CDbl(BBLVisa).ToString(FormatObject.CurrencyFormat, ci) + "</td>")
		outputString2 = outputString2.Append("</tr>")
		
		outputString2 = outputString2.Append("<tr bgColor=""" + bgColor + """>")
		outputString2 = outputString2.Append("<td align=""left"" class=""" + TextClass + """>" + "Master" + "</td>")
		outputString2 = outputString2.Append("<td align=""right"" class=""" + TextClass + """>" + CDbl(BBLMaster).ToString(FormatObject.CurrencyFormat, ci) + "</td>")
		outputString2 = outputString2.Append("</tr>")
		
		outputString2 = outputString2.Append("<tr bgColor=""" + bgColor + """>")
		outputString2 = outputString2.Append("<td align=""left"" class=""" + TextClass + """>" + "Amex" + "</td>")
		outputString2 = outputString2.Append("<td align=""right"" class=""" + TextClass + """>" + CDbl(BBLAmex).ToString(FormatObject.CurrencyFormat, ci) + "</td>")
		outputString2 = outputString2.Append("</tr>")
		
		outputString2 = outputString2.Append("<tr bgColor=""" + bgColor + """>")
		outputString2 = outputString2.Append("<td align=""left"" class=""" + TextClass + """>" + "Other" + "</td>")
		outputString2 = outputString2.Append("<td align=""right"" class=""" + TextClass + """>" + CDbl(BBLOther).ToString(FormatObject.CurrencyFormat, ci) + "</td>")
		outputString2 = outputString2.Append("</tr>")
		
		outputString2 = outputString2.Append("<tr bgColor=""#ebebeb"">")
		outputString2 = outputString2.Append("<td colspan=""2"" align=""center"" class=""" + TextClass + """>" + "Kasikorn" + "</td>")
		outputString2 = outputString2.Append("</tr>")
		
		outputString2 = outputString2.Append("<tr bgColor=""" + bgColor + """>")
		outputString2 = outputString2.Append("<td align=""left"" class=""" + TextClass + """>" + "Visa" + "</td>")
		outputString2 = outputString2.Append("<td align=""right"" class=""" + TextClass + """>" + CDbl(KBankVisa).ToString(FormatObject.CurrencyFormat, ci) + "</td>")
		outputString2 = outputString2.Append("</tr>")
		
		outputString2 = outputString2.Append("<tr bgColor=""" + bgColor + """>")
		outputString2 = outputString2.Append("<td align=""left"" class=""" + TextClass + """>" + "Master" + "</td>")
		outputString2 = outputString2.Append("<td align=""right"" class=""" + TextClass + """>" + CDbl(KBankMaster).ToString(FormatObject.CurrencyFormat, ci) + "</td>")
		outputString2 = outputString2.Append("</tr>")
		
		outputString2 = outputString2.Append("<tr bgColor=""" + bgColor + """>")
		outputString2 = outputString2.Append("<td align=""left"" class=""" + TextClass + """>" + "Amex" + "</td>")
		outputString2 = outputString2.Append("<td align=""right"" class=""" + TextClass + """>" + CDbl(KBankAmex).ToString(FormatObject.CurrencyFormat, ci) + "</td>")
		outputString2 = outputString2.Append("</tr>")
		
		outputString2 = outputString2.Append("<tr bgColor=""" + bgColor + """>")
		outputString2 = outputString2.Append("<td align=""left"" class=""" + TextClass + """>" + "Other" + "</td>")
		outputString2 = outputString2.Append("<td align=""right"" class=""" + TextClass + """>" + CDbl(KBankOther).ToString(FormatObject.CurrencyFormat, ci) + "</td>")
		outputString2 = outputString2.Append("</tr>")
		
		TotalEDC = BBLVisa + BBLMaster + BBLAmex + BBLOther + KBankVisa + KBankMaster + KBankAmex + KBankOther
		
		outputString2 = outputString2.Append("<tr bgColor=""#ebebeb"">")
		outputString2 = outputString2.Append("<td align=""left"" class=""" + TextClass + """>" + "Total EDC" + "</td>")
		outputString2 = outputString2.Append("<td align=""right"" class=""" + TextClass + """>" + CDbl(TotalEDC).ToString(FormatObject.CurrencyFormat, ci) + "</td>")
		outputString2 = outputString2.Append("</tr>")
		
		outputString2 = outputString2.Append("<tr>")
		outputString2 = outputString2.Append("<td colspan=""2"" align=""center"" class=""smallTdHeader"" bgcolor=""" + AdminBGColor + """>" + "ใบสรุปจากเครื่อง POS" + "</td>")
		outputString2 = outputString2.Append("</tr>")
		
		For i = 0 To CCTypeData.Rows.Count - 1
			outputString2 = outputString2.Append("<tr bgColor=""" + bgColor + """>")
			outputString2 = outputString2.Append("<td align=""left"" class=""" + TextClass + """>" + CCTypeData.Rows(i)("CreditCardTypeName") + "</td>")
			outputString2 = outputString2.Append("<td align=""right"" class=""" + TextClass + """>" + CDbl(CCTypeData.Rows(i)("PayAmount")).ToString(FormatObject.CurrencyFormat, ci) + "</td>")
			outputString2 = outputString2.Append("</tr>")
			SumCC += CCTypeData.Rows(i)("PayAmount")
		Next
		
		outputString2 = outputString2.Append("<tr bgColor=""" + bgColor + """>")
		outputString2 = outputString2.Append("<td align=""left"" class=""" + TextClass + """>" + "Total Credit Card" + "</td>")
		outputString2 = outputString2.Append("<td align=""right"" class=""" + TextClass + """>" + CDbl(SumCC).ToString(FormatObject.CurrencyFormat, ci) + "</td>")
		outputString2 = outputString2.Append("</tr>")
		
		outputString2 = outputString2.Append("<tr bgColor=""#ebebeb"">")
		outputString2 = outputString2.Append("<td align=""left"" class=""" + TextClass + """>" + "Diff EDC & POS" + "</td>")
		outputString2 = outputString2.Append("<td align=""right"" class=""" + TextClass + """>" + CDbl(TotalEDC-SumCC).ToString(FormatObject.CurrencyFormat, ci) + "</td>")
		outputString2 = outputString2.Append("</tr>")
		
		outputString2 = outputString2.Append("<tr bgColor=""" + bgColor + """>")
		outputString2 = outputString2.Append("<td align=""left"" class=""" + TextClass + """>" + "Cash (นับจริง)" + "</td>")
		outputString2 = outputString2.Append("<td align=""right"" class=""" + TextClass + """>" + CDbl(CashInputAmount).ToString(FormatObject.CurrencyFormat, ci) + "</td>")
		outputString2 = outputString2.Append("</tr>")
		
		outputString2 = outputString2.Append("<tr bgColor=""" + bgColor + """>")
		outputString2 = outputString2.Append("<td align=""left"" class=""" + TextClass + """>" + "Cash (จากระบบ)" + "</td>")
		outputString2 = outputString2.Append("<td align=""right"" class=""" + TextClass + """>" + CDbl(CashSystem+CashChange).ToString(FormatObject.CurrencyFormat, ci) + "</td>")
		outputString2 = outputString2.Append("</tr>")
		
		outputString2 = outputString2.Append("<tr bgColor=""#ebebeb"">")
		outputString2 = outputString2.Append("<td align=""left"" class=""" + TextClass + """>" + "Diff Cash & POS" + "</td>")
		outputString2 = outputString2.Append("<td align=""right"" class=""" + TextClass + """>" + CDbl(CashInputAmount-(CashSystem+CashChange)).ToString(FormatObject.CurrencyFormat, ci) + "</td>")
		outputString2 = outputString2.Append("</tr>")
		
		outputString2 = outputString2.Append("<tr bgColor=""#ebebeb"">")
		outputString2 = outputString2.Append("<td align=""left"" class=""" + TextClass + """>" + "Diff Total" + "</td>")
		outputString2 = outputString2.Append("<td align=""right"" class=""" + TextClass + """>" + CDbl((TotalEDC-SumCC)+(CashInputAmount-(CashSystem+CashChange))).ToString(FormatObject.CurrencyFormat, ci) + "</td>")
		outputString2 = outputString2.Append("</tr>")
		
		
		outputString2 = outputString2.Append("<tr>")
		outputString2 = outputString2.Append("<td colspan=""2"" align=""center"" class=""smallTdHeader"" bgcolor=""" + AdminBGColor + """>" + "Monthly Sales by Sale Mode" + "</td>")
		outputString2 = outputString2.Append("</tr>")
		
		outputString2 = outputString2.Append("<tr><td colspan=""2"">")
		outputString2 = outputString2.Append("<table border=""1"" cellpadding=""4"" cellspacing=""0"" style=""border-collapse:collapse;"" width=""100%"">")
		outputString2 = outputString2.Append("<tr bgColor=""#ebebeb"">")
		outputString2 = outputString2.Append("<td align=""left"" class=""" + TextClass + """>" + "Name" + "</td>")
		outputString2 = outputString2.Append("<td align=""right"" class=""" + TextClass + """>" + "Qty" + "</td>")
		outputString2 = outputString2.Append("<td align=""left"" class=""" + TextClass + """>" + "Amount" + "</td>")
		outputString2 = outputString2.Append("</tr>")
		For i = 0 To SaleModeData.Rows.Count - 1
			outputString2 = outputString2.Append("<tr bgColor=""" + bgColor + """>")
			outputString2 = outputString2.Append("<td align=""left"" class=""" + TextClass + """>" + SaleModeData.Rows(i)("SaleModeName") + "</td>")
			outputString2 = outputString2.Append("<td align=""center"" class=""" + TextClass + """>" + CDbl(SaleModeData.Rows(i)("TotalQty")).ToString(FormatObject.QtyFormat, ci) + "</td>")
			outputString2 = outputString2.Append("<td align=""right"" class=""" + TextClass + """>" + CDbl(SaleModeData.Rows(i)("SalePrice")).ToString(FormatObject.CurrencyFormat, ci) + "</td>")
			outputString2 = outputString2.Append("</tr>")
			TotalSaleModeQty += SaleModeData.Rows(i)("TotalQty")
			TotalSaleMode += SaleModeData.Rows(i)("SalePrice")
		Next
		outputString2 = outputString2.Append("<tr bgColor=""#ebebeb"">")
		outputString2 = outputString2.Append("<td align=""right"" class=""" + TextClass + """>" + "Total Monthly" + "</td>")
		outputString2 = outputString2.Append("<td align=""center"" class=""" + TextClass + """>" + CDbl(TotalSaleModeQty).ToString(FormatObject.QtyFormat, ci) + "</td>")
		outputString2 = outputString2.Append("<td align=""right"" class=""" + TextClass + """>" +  CDbl(TotalSaleMode).ToString(FormatObject.CurrencyFormat, ci) + "</td>")
		outputString2 = outputString2.Append("</tr>")
		
		outputString2 = outputString2.Append("<tr bgColor=""#ebebeb"">")
		outputString2 = outputString2.Append("<td align=""right"" class=""" + TextClass + """>" + "Avg/Day" + "</td>")
		outputString2 = outputString2.Append("<td align=""center"" class=""" + TextClass + """>" + CDbl(Noday).ToString(FormatObject.QtyFormat, ci) + "</td>")
		outputString2 = outputString2.Append("<td align=""right"" class=""" + TextClass + """>" +  CDbl(TotalSaleMode/NoDay).ToString(FormatObject.CurrencyFormat, ci) + "</td>")
		outputString2 = outputString2.Append("</tr>")
		outputString2 = outputString2.Append("</table></td></tr>")
		
		outputString2 = outputString2.Append("<tr>")
		outputString2 = outputString2.Append("<td colspan=""2"" align=""center"" class=""smallTdHeader"" bgcolor=""" + AdminBGColor + """>" + "Today Top 3 Menus" + "</td>")
		outputString2 = outputString2.Append("</tr>")
		
		outputString2 = outputString2.Append("<tr><td colspan=""2"">")
		outputString2 = outputString2.Append("<table border=""1"" cellpadding=""4"" cellspacing=""0"" style=""border-collapse:collapse;"" width=""100%"">")
		outputString2 = outputString2.Append("<tr bgColor=""#ebebeb"">")
		outputString2 = outputString2.Append("<td align=""left"" class=""" + TextClass + """>" + "Menu" + "</td>")
		outputString2 = outputString2.Append("<td align=""right"" class=""" + TextClass + """>" + "Qty" + "</td>")
		outputString2 = outputString2.Append("<td align=""left"" class=""" + TextClass + """>" + "Amount" + "</td>")
		outputString2 = outputString2.Append("</tr>")
		...
		For i = 0 To TopMenu.Rows.Count - 1
			...
			outputString2 = outputString2.Append("<td align=""center"" class=""" + TextClass + """>" + CDbl(TopMenu.Rows(i)("TotalQty")).ToString(FormatObject.QtyFormat, ci) + "</td>")
			outputString2 = outputString2.Append("<td align=""right"" class=""" + TextClass + """>" + CDbl(TopMenu.Rows(i)("SalePrice")).ToString(FormatObject.CurrencyFormat, ci) + "</td>")
			outputString2 = outputString2.Append("</tr>")
		Next
		...
		outputString3 = outputString3.Append("<td colspan=""2"" align=""center"" class=""smallTdHeader"" bgcolor=""" + AdminBGColor + """>" + "Statistics" + "</td>")
		outputString3 = outputString3.Append("</tr>")
		
		outputString3 = outputString3.Append("<tr>")
		outputString3 = outputString3.Append("<td align=""center"" class=""smallTdHeader"" bgcolor=""" + AdminBGColor + """>" + "Void Bill(s)" + "</td>")
		outputString3 = outputString3.Append("<td align=""center"" class=""smallTdHeader"" bgcolor=""" + AdminBGColor + """>" + "Void Amount" + "</td>")
		outputString3 = outputString3.Append("</tr>")
		
		outputString3 = outputString3.Append("<tr bgColor=""" + bgColor + """>")
		outputString3 = outputString3.Append("<td align=""right"" class=""" + TextClass + """>" + CDbl(VoidQty).ToString(FormatObject.QtyFormat, ci) + "</td>")
		outputString3 = outputString3.Append("<td align=""right"" class=""" + TextClass + """>" + CDbl(VoidAmount).ToString(FormatObject.CurrencyFormat, ci) + "</td>")
		outputString3 = outputString3.Append("</tr>")
		
		outputString3 = outputString3.Append("<tr>")
		outputString3 = outputString3.Append("<td align=""center"" class=""smallTdHeader"" bgcolor=""" + AdminBGColor + """>" + "Tip Person" + "</td>")
		outputString3 = outputString3.Append("<td align=""center"" class=""smallTdHeader"" bgcolor=""" + AdminBGColor + """>" + "Tip Amount" + "</td>")
		outputString3 = outputString3.Append("</tr>")
		
		outputString3 = outputString3.Append("<tr bgColor=""" + bgColor + """>")
		outputString3 = outputString3.Append("<td align=""right"" class=""" + TextClass + """>" + CDbl(TipPerson).ToString(FormatObject.QtyFormat, ci) + "</td>")
		outputString3 = outputString3.Append("<td align=""right"" class=""" + TextClass + """>" + CDbl(TipAmount).ToString(FormatObject.CurrencyFormat, ci) + "</td>")
		outputString3 = outputString3.Append("</tr>")
		
		outputString3 = outputString3.Append("<tr>")
		outputString3 = outputString3.Append("<td colspan=""2"" align=""center"" class=""smallTdHeader"" bgcolor=""" + AdminBGColor + """>" + "Remark" + "</td>")
		outputString3 = outputString3.Append("</tr>")
		outputString3 = outputString3.Append("<tr bgColor=""" + bgColor + """>")
		outputString3 = outputString3.Append("<td colspan=""2"" align=""left"" class=""" + TextClass + """>" + RemarkText + "</td>")
		outputString3 = outputString3.Append("</tr>")
		
		outputString3 = outputString3.Append("<tr>")
		outputString3 = outputString3.Append("<td colspan=""2"" align=""center"" class=""smallTdHeader"" bgcolor=""" + AdminBGColor + """>" + "สรุปการปิดกะ" + "</td>")
		outputString3 = outputString3.Append("</tr>")
