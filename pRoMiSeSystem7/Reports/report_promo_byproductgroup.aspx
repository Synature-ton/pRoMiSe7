...
<html>
<head>
<title>Promotion Discount By Product Group Reports</title>
...
Sub Page_Load()
	If User.Identity.IsAuthenticated  AND Session("Report_ByProductGroup") Then
	...
	Try	
		...
		HeaderString += "<td align=""center"" class=""smallTdHeader"" bgcolor=""" + GlobalParam.AdminBGColor + """>" + "# Coupon" + "</td>"	
		...
		If Not Page.IsPostBack Then
			...
			If IsNumeric(Request.QueryString("ShopID")) Then
				...
				'Application.Lock()
				Dim Result As String = getReport.PromotionDiscountReport(SummaryPromotion,2,dtTable,PromotionData,Request.QueryString("StartDate"), Request.QueryString("EndDate"), Request.QueryString("ShopID"), Session("LangID"), 0, objCnn)	
				'Application.UnLock()
		...
		Dim ShopData As DataTable = getInfo.GetProductLevelAccess(-999,Session("StaffRole"),objCnn) 'getInfo.GetAllShopData(objCnn)
...
Sub DoSearch(Source As Object, E As EventArgs)
	...
	If FoundError = False Then
		...
		Dim SummaryPromotion As New DataTable()
		'Application.Lock()
		Dim Result As String = Reports.PromotionDiscountReport(SummaryPromotion,2,dtTable,PromotionData,StartDate, EndDate, Request.Form("ShopID"), Session("LangID"), PromotionID, objCnn)
		'Application.UnLock()
		'Exit Sub
...
Public Function GenResults(ByVal PromotionData As DataTable, ByVal ShopID As Integer, ByVal ReportDate As String, ByVal LangDefault As DataTable, ByVal LangData2 As DataTable) As String
	...
	Dim ci As New CultureInfo(FormatObject.CultureString)
		...
		End If
		ResultSearchText.InnerHtml = ReportHeader + LangData2.Rows(6)(LangText) + " (" + ReportDate + ")"
		
		Dim DummyProductGroupID As Integer = -1
		Dim DummyProductGroupCode As String = ""
		Dim netPrice As Double = 0
		Dim totalQty As Double = 0
		Dim totalNo As Double = 0
		...
		For i = 0 To PromotionData.Rows.Count - 1
			...
			If Not IsDBNull(PromotionData.Rows(i)("TotalQty")) Then
				grandTotalNo += PromotionData.Rows(i)("TotalQty")
			End If
		Next
        For i = 0 To PromotionData.Rows.Count - 1
            If PromotionData.Rows(i)("ProductGroupCode") <> DummyProductGroupCode Then
                If i > 0 Then
                    outputString = outputString.Append("<tr bgColor=""" + GlobalParam.GrayBGColor + """>")
                    outputString = outputString.Append("<td colspan=""2"" align=""right"" class=""smallText"">" + LangData2.Rows(7)(LangText) + "</td>")
                    outputString = outputString.Append("<td align=""center"" class=""smallText"">" + Format(totalNo, FormatData.Rows(0)("QtyFormat")) + "</td>")
                ...
                End If
                outputString = outputString.Append("<tr>")
                If PromotionData.Rows(i)("ProductGroupCode") <> "" Then
                    outputString = outputString.Append("<td colspan=""" + ColSpan + """ align=""left"" class=""smallTdHeader"" bgcolor=""" + GlobalParam.AdminBGColor + """>" + LangData2.Rows(8)(LangText) + ": " + PromotionData.Rows(i)("ProductGroupName") + "</td>")
                Else
                    outputString = outputString.Append("<td colspan=""" + ColSpan + """ align=""left"" class=""smallTdHeader"" bgcolor=""" + GlobalParam.AdminBGColor + """>" + LangData2.Rows(9)(LangText) + "</td>")
                End If
                outputString = outputString.Append("</tr>")
                countPromo += 1
            End If
            ...
            End If
            If Not IsDBNull(PromotionData.Rows(i)("TotalQty")) Then
                outputString = outputString.Append("<td align=""center"" class=""smallText"">" + Format(PromotionData.Rows(i)("TotalQty"), FormatData.Rows(0)("QtyFormat")) + "</td>")
            Else
            ...
            DummyProductGroupCode = PromotionData.Rows(i)("ProductGroupCode")
            If Not IsDBNull(PromotionData.Rows(i)("TotalQty")) Then
                totalNo += PromotionData.Rows(i)("TotalQty")
            End If
        Next
		If i > 0 Then
			outputString = outputString.Append("<tr bgColor=""" + GlobalParam.GrayBGColor + """>")
		...
		If countPromo > 1 Then
			outputString = outputString.Append("<tr><td height=""10px"" colspan=""7""></td></tr>")
			outputString = outputString.Append("<tr bgColor=""" + GlobalParam.GrayBGColor + """>")
			outputString = outputString.Append("<td colspan=""2"" align=""right"" class=""smallText"">" + LangData2.Rows(10)(LangText) + "</td>")
			'outputString = outputString.Append("<td align=""center"" class=""smallText"">" + Format(grandtotalNo, FormatData.Rows(0)("QtyFormat")) + "</td>")
