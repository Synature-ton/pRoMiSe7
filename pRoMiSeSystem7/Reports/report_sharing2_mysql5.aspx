...
<html>
<head>
<title>Sharing Sale Reports</title>
...
<tr id="ShowShop" runat="server">
    ...
    <td>
	    <table>
            ...
            <tr>
                <td></td>
                <td class="text"><div id="DocumentToDateParam" class="text" runat="server"></div>
		        <synature:date id="ToDate" runat="server" /></td>
            </tr>
	        <tr>
               <td colspan="4" class="text">&nbsp;<asp:CheckBox ID="DisplayGraph" CssClass="text" Checked="true" runat="server" />
                   &nbsp;&nbsp;<asp:radiobutton ID="Piechart" Text="Pie Chart" Checked="true" GroupName="Group2" CssClass="text" runat="server" />
			       &nbsp;&nbsp;<asp:radiobutton ID="Barchart" Text="Column Chart" GroupName="Group2" CssClass="text" runat="server" />
               </td>
            </tr>
            <tr>
               <td colspan="4" class="text">
                 <asp:button ID="SubmitForm" Font-Size="8" Height="20" Width="110" OnClick="DoSearch" runat="server" />
                </td>
	        </tr>       
	    </table>
	</td>
...
Dim FormatDocNumber As New FormatText()
    ...
    Sub Page_Load()
        If User.Identity.IsAuthenticated And Session("Report_Sharing2") Then
            ...
            headerPricePerUnit.Visible = False
            ...
            txtProductName.InnerHtml = "Name"
            txtPricePerUnit.InnerHtml = "Unit Price"
    ...
    Sub DoSearch(Source As Object, E As EventArgs)
        ...
        Dim dclGrandTotalPrice, dclGrandDiscountPrice, dclGrandTotalSalePrice, dclGrandAmount, dclPrice As Decimal
        Dim GraphData As New DataSet()
        Dim bolDisplayPricePerUnitColumn As Boolean
        ...
        If FoundError = False Then
            ...
            If Not (cboGroupInfo.SelectedItem.Value = "0" Or cboDeptInfo.SelectedItem.Value = "0") Then
                bolDisplayPricePerUnitColumn = True
                colSpan = 4
                headerPricePerUnit.Visible = True
            Else
                bolDisplayPricePerUnitColumn = False
                colSpan = 3
                headerPricePerUnit.Visible = False
            End If
            
            'Application.Lock()
            Dim bolOrderBySalePercent As Boolean
            dclGrandAmount = 0
            dclGrandTotalPrice = 0
            dclGrandTotalSalePrice = 0
            bolOrderBySalePercent = True
            getReport.SharingSaleReports(dtTable, dclGrandAmount, dclGrandTotalPrice, dclGrandTotalSalePrice, GraphData, StartDate, EndDate, _
                          strShopID, cboGroupInfo.SelectedItem.Value, cboDeptInfo.SelectedItem.Value, DisplayGraph.Checked, Session("LangID"), _
                          "", 1, bolOrderBySalePercent, objCnn)
            dclGrandDiscountPrice = dclGrandTotalPrice - dclGrandTotalSalePrice
            ...
            Dim dtTableData As DataTable = gData.Tables.Add("Data")
            dtTableData.Columns.Add("Description")
            dtTableData.Columns.Add("Value1", GetType(Double))
            Dim subTotal As Double = 0
            Dim SaleTotal As Double = 0
					
            noRow = 0
            For i = 0 To dtTable.Rows.Count - 1
                If dtTable.Rows(i)("TotalSale") <> 0 Then
                    noRow += 1
                    strBuild.Append("<tr>")
                    strBuild.Append("<td align=""left"" class=""" + TextClass + """>" & noRow & "</td>")
                    'ProductGroup or ProductDept or Product Code
                    If Not IsDBNull(dtTable.Rows(i)("PCode")) Then
                        strBuild.Append("<td align=""left"" class=""" + TextClass + """>" & dtTable.Rows(i)("PCode") & "</td>")
                    Else
                        strBuild.Append("<td align=""left"" class=""" + TextClass + """>" & "-" & "</td>")
                    End If
                    'ProductGroup or ProductDept or Product Name
                    If Not IsDBNull(dtTable.Rows(i)("PName")) Then
                        strBuild.Append("<td align=""left"" class=""" + TextClass + """>" & dtTable.Rows(i)("PName") & "</td>")
                    Else
                        strBuild.Append("<td align=""left"" class=""" + TextClass + """>" & "-" & "</td>")
                    End If
                    'Product Price Per Unit (Only Product)
                    If bolDisplayPricePerUnitColumn = True Then
                    ...
                    'Percent
                    If dclGrandTotalSalePrice = 0 Then
                        strBuild.Append("<td align=""right"" class=""" + TextClass + """>" & "-" & "</td>")
                    Else
                        strBuild.Append("<td align=""right"" class=""" + TextClass + """>" & Format(dtTable.Rows(i)("TotalSale") * 100 / dclGrandTotalSalePrice, "##,##0.00") & "%</td>")
                    End If
                    Dim row As DataRow = dtTableData.NewRow()
                    row("Description") = dtTable.Rows(i)("PName")
                    row("Value1") = Format(dtTable.Rows(i)("TotalSale") * 100 / dclGrandTotalSalePrice, "##,##0.00")
                    dtTableData.Rows.Add(row)
            ...
            If noRow <> 0 Then
                ...
                strBuild.Append("<td align=""right"" class=""" + TextClass + """>" & Format(dclGrandAmount, "##,##0.00") & "</td>")
                ...
                strBuild.Append("<td align=""right"" class=""" + TextClass + """>" & Format(dclGrandTotalSalePrice, "##,##0.00") & "</td>")
                If dclGrandTotalSalePrice = 0 Then
                    strBuild.Append("<td align=""right"" class=""" + TextClass + """>" & "-" & "</td>")
                Else
                    strBuild.Append("<td align=""right"" class=""" + TextClass + """>" & Format(dclGrandTotalSalePrice * 100 / dclGrandTotalSalePrice, "##,##0.00") & "%</td>")
                End If
                strBuild.Append("</tr>")
            End If
            ...
            If cboDeptInfo.SelectedItem.Value <> "0" Then
                ...
                HeaderGroup = " Product "
            End If
            ResultSearchText.InnerHtml = "Sales Contribution : " & HeaderGroup & "of " & strShopGroupDept & " (" + ReportDate + ")"
            If DisplayGraph.Checked = True Then
                ...
                If Barchart.Checked = True Then
                    ...
                    ConfigureColors("Sale Report By" + HeaderGroup + "of " & strShopGroupDept & " (" + ReportDate + ")")
                ...
                Else
                    ...
                    chart.DataBind()
                    ChartControl1.Charts.Add(chart)
                    ConfigureColors("Sale Report By Group of " & strShopGroupDept & " (" + ReportDate + ")")
