...
Sub DoSearch(Source As Object, E As EventArgs)
	...
	If FoundError = False Then
		...
		End If
		ResultSearchText.InnerHtml = LangData2.Rows(123)(LangText) + " " + ShopDisplay + " (" + ReportDate + ReportTime + ")"
...
End Sub
 ...
 Sub ConfigureColors(TitleName)
    ...
    End Sub
        ...
        If ReportByProduct = True Or (ReportByBill = True And ShopID > 0 And ViewOption = 0 And ExpandReceipt = True) Then
            ...
            If ReportByProduct = True Then
                ...
                outputString = ""
                grandTotal = 0
                SaleReportByProduct(GraphData, outputString, grandTotal, VATTotal, ShowSummary, GrayBGColor, AdminBGColor, ShopID, ViewOption, StartDate, EndDate, GetData, objDB.List(sqlStatement1, objCnn), PayByCreditMoney, LangID, LangPath, TimeString, objCnn)
                objDB.sqlExecute("DROP TABLE IF EXISTS DummyGroupOrdering_" + ExtraTableString, objCnn)
                objDB.sqlExecute("DROP TABLE IF EXISTS DummyDeptOrdering_" + ExtraTableString, objCnn)
  ...
  Public Function SaleReportByProduct(ByRef GraphData As DataSet, ByRef ResultString As String, ByRef grandTotal As Double, ByRef VATTotal As Double, ByVal ShowSummary As Boolean, ByVal GrayBGColor As String, ByVal AdminBGColor As String, ByVal ShopID As Integer, ByVal ViewOption As Integer, ByVal StartDate As String, ByVal EndDate As String, ByVal dtTable As DataTable, ByVal PaymentResult As DataTable, ByVal PayByCreditMoney As DataTable, ByVal LangID As Integer, ByVal LangPath As String, ByVal TimeString As String, ByVal objCnn As MySqlConnection) As String
        ...
        If ShowSummary = True Then
            ...
            Dim serviceCharge As DataTable = objDB.List("select SUM(a.ServiceCharge) AS ServiceCharge, SUM(a.ServiceChargeVAT) AS ServiceChargeVAT from  ordertransaction a WHERE TransactionStatusID=2 AND ReceiptID > 0 AND SaleDate >= " + StartDate + " AND SaleDate < " + EndDate + ServiceChargeQuery + TimeString, objCnn)
            ...
            Else
                ...
                Dim VATData As DataTable = objDB.List("select SUM(a.TransactionExcludeVAT) AS TotalVAT from  ordertransaction a WHERE TransactionStatusID=2 AND ReceiptID > 0 AND SaleDate >= " + StartDate + " AND SaleDate < " + EndDate + ServiceChargeQuery + TimeString, objCnn)
                outputString = outputString.Append("<tr bgColor=""" + GrayBGColor + """>")
            ...
            If dtTable.Rows.Count > 0 Then
                Dim ResultSummary As String = SaleReportSummaryNew(grandTotalSaleAvg, grandTotalRetailPrice, FinalSaleAmount, GrayBGColor, AdminBGColor, ShopID, ViewOption, StartDate, EndDate, grandTotalVAT, grandTotalAfterVAT, grandTotalVoid, PaymentResult, PayByCreditMoney, DiscountArray, True, False, LangID, LangPath, objCnn)
    ...
    End Function
	   ...
	   Public Overloads Function SaleReportSummaryNew(ByVal grandTotalSaleAvg As Double, ByVal grandTotalRetailPrice As Double, ByRef FinalSaleAmount As Double, ByVal GrayBGColor As String, ByVal AdminBGColor As String, ByVal ShopID As Integer, ByVal ViewOption As Integer, ByVal StartDate As String, ByVal EndDate As String, ByVal grandTotalVAT As Double, ByVal grandTotalAfterVAT As Double, ByVal grandTotalVoid As Double, ByVal PaymentResult As DataTable, ByVal PayByCreditMoney As DataTable, ByVal DiscountArray() As Double, ByVal ShowStatistics As Boolean, ByVal VATDisplay As Boolean, ByVal LangID As Integer, ByVal LangPath As String, ByVal objCnn As MySqlConnection) As String
        ...
        If ShowStatistics = True Then
            ...
            Dim AvgBill As DataTable = objDB.List("select count(*) AS TotalBill, SUM(NoCustomer) AS TotalCustomer from OrderTransaction where (SaleDate >= " + StartDate + " and SaleDate < " + EndDate + ") AND TransactionStatusID=2 AND DocType IN (4,8) AND ReceiptID>0 AND ShopID IN (" + ShopID.ToString + ")", objCnn)
            Dim ChkAvgBill As DataTable = objDB.List("select count(*) AS MinusBill, SUM(NoCustomer) AS MinusCustomer from OrderTransaction a left outer join orderdetail b on a.transactionid=b.transactionid and a.computerid=b.computerid where b.transactionid is null AND (SaleDate >= " + StartDate + " and SaleDate < " + EndDate + ") AND TransactionStatusID=2 AND DocType IN (4,8) AND ReceiptID>0 AND ShopID IN (" + ShopID.ToString + ")", objCnn)
            Dim VoidBill As DataTable = objDB.List("select count(*) AS VoidBill from OrderTransaction where (SaleDate >= " + StartDate + " and SaleDate < " + EndDate + ") AND TransactionStatusID<>2 AND ReceiptID>0 AND DocType IN (4,8) AND ShopID IN (" + ShopID.ToString + ")", objCnn)
            Dim totalQty As DataTable = objDB.List("select SUM(b.Amount) AS TotalQty from OrderTransaction a, OrderDetail b where a.TransactionID=b.TransactionID AND a.ComputerID=b.ComputerID AND (SaleDate >= " + StartDate + " and SaleDate < " + EndDate + ") AND TransactionStatusID=2 AND DocType IN (4,8) AND ReceiptID>0 AND b.OrderStatusID<>3 AND a.ShopID IN (" + ShopID.ToString + ")", objCnn)
