...
<tr id="ShowShop" runat="server">
	...
	<table>
		...
		<td colspan="2"><asp:CheckBox ID="ExpandReceipt" CssClass="text" Checked="false" Visible="false" runat="server" /><asp:CheckBox ID="ShowSummary" CssClass="text" Checked="True" Visible="True" Text="Show Summary Statistics" runat="server" /></td>
		</tr>
		<tr>
...
<span id="startTable" runat="server"></span>
    ...
    <div id="SCSummary" runat="server"></div>
...
Sub Page_Load()
	If User.Identity.IsAuthenticated  AND Session("Report_Sale_New2") Then
...
Sub DoSearch(Source As Object, E As EventArgs)
	...
	ElseIf Request.Form("Group2") = "Radio_13" Then
		R13 = True
		If Request.Form("GroupByParam") = 1 Then
			ReportType = "By Day Of Week"
		Else
			ReportType = "By Hourly"
		End If
	End If
	...
	Dim ShowStat As Boolean = False
	If Request.Form("ShowSummary") = "on" Then ShowStat = True
	...
	If FoundError = False Then
		...
		If Request.Form("ShopID") > 0 Or (Request.Form("ShopID")=0 AND R12 = True) Then
			ResultText.InnerHtml = getReport.SaleReports(PayTypeList,outputString, grandTotal, VATTotal, GraphData, True,GlobalParam.GrayBGColor,GlobalParam.AdminBGColor,Session("LangID"),ViewOption,R11, R12, R13, Request.Form("GroupByParam"), StartDate, EndDate,"","","","", Request.Form("ShopID"),0,0,ShowStat,ExpReceipt, DGraph,1, objCnn)	
		Else
		...
		If R12 = False Then
			SCSummary.InnerHtml = ServiceChargeSummary(GlobalParam.GrayBGColor,GlobalParam.AdminBGColor,Request.Form("ShopID"), StartDate, EndDate,objCnn)
		End If
		...
		If (R11 = True AND (R1 = True Or R2 = True Or R4 = True)) Or R13 = True Or (R11 = True AND R3 = True And ExpReceipt = False) Then
				...
				If AllShop = True Then
					AdditionalHeader += "<td align=""center"" class=""smallTdHeader"" bgcolor=""" + GlobalParam.AdminBGColor + """>" + "No. of Pax" + "</td>"
					AdditionalHeader += "<td align=""center"" class=""smallTdHeader"" bgcolor=""" + GlobalParam.AdminBGColor + """>" + "Spending per Pax" + "</td>"
				End If
		...
		ResultSearchText.InnerHtml = "Sale Report " + ReportType + " of " + ShopDisplay + " (" + ReportDate + ")"
		...
		If DGraph = True AND (R11 = True Or R13 = True) Then
...
End Sub
 ...
 Sub ConfigureColors(TitleName)
	...
	Public Function ServiceChargeSummary(ByVal GrayBGColor As String, ByVal AdminBGColor As String, ByVal ShopID As Integer, ByVal StartDate As String, ByVal EndDate As String, ByVal objCnn As MySqlConnection) As String
        Dim i As Integer
        Dim ii As Integer
        ...
        AdditionalQuery += " AND a.DocType=8 AND a.TransactionStatusID=2 AND a.ReceiptID>0"

        sqlStatement = "select a.PayTypeID,a.PayTypeName,SUM(a.PaidAmount*(a.ServiceCharge+a.ServiceChargeVAT)/d.PayAmount) As ServiceChargePay from DummyTranPayment a, (select a.TransactionID,a.ComputerID,SUM(a.PaidAmount) As PayAmount From DummyTranPayment a where 0=0 " + AdditionalQuery + " group by a.TransactionID,a.ComputerID) d where a.TransactionID=d.TransactionID AND a.ComputerID=d.ComputerID " + AdditionalQuery + " group by a.PayTypeID,a.PayTypeName order by a.PayTypeID"
        ...
        outputString += "<td colspan=""" + "3" + """ align=""center"" class=""tdHeader"">Service Charge Summary</td>"
        outputString += "</tr>"
        outputString += "<tr bgColor=""" + AdminBGColor + """>"
        outputString += "<td align=""center"" class=""tdHeader"">Payment Type</td>"
        outputString += "<td align=""center"" class=""tdHeader"">Service Charge</td>"
        outputString += "<td align=""center"" class=""tdHeader"">%</td>"
        outputString += "</tr>"
        Dim dtTable As DataTable = objDB.List(sqlStatement, objCnn)
        TextClass = "smallText"
        Dim totalSC As Double = 0
        For i = 0 To dtTable.Rows.Count - 1
			If Not IsDBNull(dtTable.Rows(i)("ServiceChargePay")) Then
            	totalSC += dtTable.Rows(i)("ServiceChargePay")
			End If
        Next
		...
		If Not IsDBNull(dtTable.Rows(i)("ServiceChargePay")) Then
            outputString += "<tr>"
            outputString += "<td align=""right"" class=""" + TextClass + """>" + dtTable.Rows(i)("PayTypeName") + "</td>"
            outputString += "<td class=""" + TextClass + """ align=""right"">" + Format(dtTable.Rows(i)("ServiceChargePay"), "##,##0.00") + "</td>"
            outputString += "<td class=""" + TextClass + """ align=""right"">" + Format(dtTable.Rows(i)("ServiceChargePay") * 100 / totalSC, "##,##0.00") + "%</td>"
            outputString += "</tr>"
		End If
        Next

        outputString += "<tr bgColor=""" + GrayBGColor + """>"
        outputString += "<td align=""right"" class=""" + TextClass + """>" + "Total" + "</td>"
        outputString += "<td class=""" + TextClass + """ align=""right"">" + Format(totalSC, "##,##0.00") + "</td>"
        outputString += "<td class=""" + TextClass + """ align=""right"">" + Format(100, "##,##0.00") + "%</td>"
        outputString += "</tr>"
        outputString += "</table>"
