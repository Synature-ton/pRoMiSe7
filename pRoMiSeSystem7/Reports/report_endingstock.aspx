...
Sub DoSearch(Source As Object, E As EventArgs)
	...
	If FoundError = False Then
		...
		'Application.Lock()
			Result = EndingStockReport(dtTable,TableNameString,Request.Form("ShopID"),SelMonth, SelYear,Request.Form("OrderParam"),OrderParam.SelectedIndex,objCnn)
		'Application.UnLock()
...
Public Function EndingStockReport(ByRef dtTable As DataTable, ByVal TableNameString As String, ByVal ShopID As Integer, ByVal SelMonth As Integer, ByVal SelYear As Integer, ByVal OrderParam As String, ByVal OrderingIndex As Integer, ByVal objCnn As MySqlConnection) As String
		...
		Dim SelectString As String = "mg.MaterialGroupID,mg.MaterialGroupCode,mg.MaterialGroupName"
		Dim JoinTable As String = ""
		
		If OrderingIndex > 1 Then
			SelectString = "m.MaterialAccountID As MaterialGroupID,MaterialAccountCode As MaterialGroupCode,MaterialAccountName As MaterialGroupName"
			JoinTable = " left outer join MaterialAccountData ac ON m.MaterialAccountID=ac.MaterialAccountID"
		End If
        ...
        TestTime += "<br>" + DateTimeUtil.CurrentDateTime
        sqlStatement = " select " + SelectString + ",IF(aaa.UnitSmallRatio is not NULL,b.ProductNetPrice*aaa.UnitSmallRatio/b.UnitSmallAmount,b.ProductNetPrice*c.UnitSmallRatio/b.UnitSmallAmount) As CalPricePerUnit, b.ProductPricePerUnit,IF(aaa.UnitLargeName is not NULL, aaa.UnitLargeName, UnitName) As UnitName,ProductAmount,MaterialCode,MaterialName,b.ProductNetPrice,b.UnitSmallAmount,IF(aaa.UnitSmallRatio is not NULL,aaa.UnitSmallRatio,c.UnitSmallRatio) As UnitSmallRatio FROM Document a left outer join DocDetail b ON a.DocumentID=b.DocumentID AND a.ShopID=b.ShopID left outer join documenttype dt ON a.DocumentTypeID=dt.DocumentTypeID AND a.ProductLevelID=dt.ShopID left outer join materials m ON b.ProductID=m.MaterialID left outer join ProductLevel pl ON a.ProductLevelID=pl.ProductLevelID left outer join MaterialDept md ON m.MaterialDeptID=md.MaterialDeptID left outer join MaterialGroup mg ON md.MaterialGroupID=mg.MaterialGroupID left outer join UnitRatio c ON b.UnitID=c.UnitLargeID and b.ProductUnit=c.UnitSmallID left outer join (select MaterialID,ur.UnitLargeID,mu.UnitSmallID,UnitSmallRatio,ul.UnitLargeName from materialdocumenttypeunitsetting mu, UnitRatio ur, UnitLarge ul where mu.SelectUnitLargeID=ur.UnitLargeID AND mu.UnitSmallID=ur.UnitSmallID AND ur.UnitLargeID=ul.UnitLargeID AND mu.DocumentTypeID=7 AND IsDefault=1) aaa ON m.MaterialID=aaa.MaterialID  " + JoinTable + " where a.DocumentStatus=2 AND dt.LangID=1 AND a.DocumentTypeID=10 AND b.UnitSmallAmount<>0 AND MONTH(a.DocumentDate)=" + EndMonthValue.ToString + " AND YEAR(a.DocumentDate)=" + EndYearValue.ToString + AdditionalQuery + " order by " + OrderParam
