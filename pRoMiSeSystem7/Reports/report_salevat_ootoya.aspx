...
Sub DoSearch(Source As Object, E As EventArgs)
	...
	If FoundError = False Then
		...
		'Application.Lock()
		Dim dtTable As DataTable = SaleVATReports(StartDate, EndDate, Request.Form("ShopID"), Session("LangID"), ViewOption, objCnn)
...
Public Function SaleVATReports(ByVal StartDate As String, ByVal EndDate As String, ByVal ShopID As Integer, ByVal LangID As Integer, ByVal ViewOption As Integer, ByVal objCnn As MySqlConnection) As DataTable
        ...
        If PropertyInfo.Rows(0)("FrontSystemType") = 1 Then
            If ShopID = 0 Then
                ...
                sqlStatement = "select a.ShopID,pl.ProductLevelName AS ShopName,a.CloseComputerID AS ComputerID,t.RegistrationNumber,e.ReceiptID,e.ReceiptMonth,e.ReceiptYear,e.MaxReceiptID,e.MaxReceiptMonth,e.MaxReceiptYear,DocumentTypeHeader,SUM(a.TransactionVatable) AS TransactionVatable,SUM(a.TransactionVAT) AS TransactionVAT from ordertransaction a inner join (select a.ShopID,a.CloseComputerID As ComputerID,MIN(a.ReceiptID) AS ReceiptID,MIN(a.ReceiptMonth) AS ReceiptMonth,MIN(a.ReceiptYear) AS ReceiptYear,MAX(a.ReceiptID) AS MaxReceiptID,MAX(a.ReceiptMonth) AS MaxReceiptMonth,MAX(a.ReceiptYear) AS MaxReceiptYear From OrderTransaction a WHERE a.ReceiptID > 0 " + AdditionalQuery + " group by a.ShopID,a.CloseComputerID) e ON a.ShopID=e.ShopID AND a.CloseComputerID=e.ComputerID inner join ComputerName t ON a.CloseComputerID=t.ComputerID inner join ProductLevel pl ON a.ShopID=pl.ProductLevelID left outer join DocumentType dt ON a.DocType=dt.DocumentTypeID AND a.CloseComputerID=dt.ComputerID  where a.TransactionStatusID=2 AND a.ReceiptID > 0 AND dt.LangID=" + LangID.ToString + AdditionalQuery + " group by a.ShopID,pl.ProductLevelName,a.CloseComputerID,t.RegistrationNumber Order By a.ShopID,a.CloseComputerID"
            Else
                sqlStatement = "select a.SaleDate,a.CloseComputerID AS ComputerID,t.RegistrationNumber,e.ReceiptID,e.ReceiptMonth,e.ReceiptYear,e.MaxReceiptID,e.MaxReceiptMonth,e.MaxReceiptYear,DocumentTypeHeader,SUM(a.TransactionVatable) AS TransactionVatable,SUM(a.TransactionVAT) AS TransactionVAT from ordertransaction a inner join (select a.SaleDate,a.CloseComputerID AS ComputerID,MIN(a.ReceiptID) AS ReceiptID,MIN(a.ReceiptMonth) AS ReceiptMonth,MIN(a.ReceiptYear) AS ReceiptYear,MAX(a.ReceiptID) AS MaxReceiptID,MAX(a.ReceiptMonth) AS MaxReceiptMonth,MAX(a.ReceiptYear) AS MaxReceiptYear From OrderTransaction a WHERE a.ReceiptID > 0 " + AdditionalQuery + " group by a.SaleDate,a.CloseComputerID) e ON a.SaleDate=e.SaleDate AND a.CloseComputerID=e.ComputerID inner join ComputerName t ON a.CloseComputerID=t.ComputerID left outer join DocumentType dt ON a.DocType=dt.DocumentTypeID AND a.CloseComputerID=dt.ComputerID  where a.TransactionStatusID=2 AND a.ReceiptID > 0 AND dt.LangID=" + LangID.ToString + AdditionalQuery + " group by a.SaleDate,a.CloseComputerID,t.RegistrationNumber Order By a.SaleDate,a.CloseComputerID"
            End If
        Else
