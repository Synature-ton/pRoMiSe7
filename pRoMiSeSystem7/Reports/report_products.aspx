...
Sub Page_Load()
	If User.Identity.IsAuthenticated  AND Session("Reports_Products") Then
	...
	Try	
		...
		Dim HeaderReport As DataTable = getProp.PermissionName(1021,Session("LangID"),objCnn)
		
		If HeaderReport.Rows.Count > 0 Then
			HeaderText.InnerHtml = HeaderReport.Rows(0)("PermissionItemName")
		Else
			HeaderText.InnerHtml = "Product Reports"
		End If
		...
		rData = DocTypeGroup.NewRow
		rData("DocumentTypeGroupID") = "25,1007,1008"
		...
		If DocTypeGroup.Rows.Count > 0 Then
			...
			outputString = "<select name=""DocumentTypeGroupID"" style=""width:200px"">"
		...
		Dim ReportType As DataTable = ReportProductList("",objCnn)
		
		outputString = "<select name=""ReportTypeID"" style=""width:200px"">"
		
		For i = 0 to ReportType.Rows.Count - 1
			If ReportTypeID = ReportType.Rows(i)("ListID") Then
				FormSelected = "selected"
			Else
				FormSelected = ""
			End If
			outputString += "<option value=""" & ReportType.Rows(i)("ListID") & """ " & FormSelected & ">" & ReportType.Rows(i)("ListName")
		Next
		outputString += "</select>"
...
Sub DoSearch(Source As Object, E As EventArgs)
	...
	If FoundError = False Then
		...
		If Request.Form("DocumentTypeGroupID") = "3"  or Request.Form("DocumentTypeGroupID") = "25,1007,1008" Then
		...
		Dim TableNameString As String = "ReportProduct_" + Session.SessionID
		...
		If Request.Form("DocumentTypeGroupID") = "3" Then
		...
		Else
			...
			TestTime += "<br>" + DateTimeUtil.CurrentDateTime
				Dim DeptCode As String = "'0031','0041'"
				Dim ReportType As DataTable = ReportProductList(Request.Form("ReportTypeID").ToString,objCnn)
				If ReportType.Rows.Count > 0 Then
					DeptCode = ReportType.Rows(0)("ListValue")
				End If
				displayTable = objDB.List("select * from " + TableNameString + " a,MaterialDept b where a.MaterialDeptID=b.MaterialDeptID AND b.MaterialDeptCode IN (" + DeptCode + ") order by MaterialCode", objCnn)
				...
				For i = 0 To displayTable.Rows.Count - 1
					...
					UnitRatioVal = 1
					UnitText = displayTable.Rows(i)("UnitSmallName")
					UnitInfo = getInfo.GetUnitSetting(displayTable.Rows(i)("MaterialID"),displayTable.Rows(i)("UnitSmallID"),objCnn)
				   ...
				   outputString = outputString.Append("<td align=""center"" class=""" + TextClass + """>" + UnitText + "</td>")  
				   For j = 0 To ShopData.Rows.Count - 1 
				   		If UnitRatioVal = 1 Or displayTable.Rows(i)("Shop_" + ShopData.Rows(j)("ProductLevelID").ToString) = 0 Then
						...
						Else
							outputString = outputString.Append("<td align=""right"" class=""" + TextClass + """><a class=""smalltext"" href=""JavaScript: newWindow = window.open( 'report_material_doctype.aspx?MaterialID=" + displayTable.Rows(i)("MaterialID").ToString + "&StartDate=" & Replace(StartDate,"'","\'") & "&EndDate=" & Replace(EndDate,"'","\'") & "&ProductLevelID=" & ShopData.Rows(j)("ProductLevelID").ToString & "&ReportDate=" & ReportDate & "&DocumentTypeID=" & Request.Form("DocumentTypeGroupID").ToString + "', '', 'width=800,height=650,toolbar=0,location=0,directories=0,status=0,menuBar=0,scrollBars=1,resizable=1' ); newWindow.focus()"">" + Format(Math.ABS(displayTable.Rows(i)("Shop_" + ShopData.Rows(j)("ProductLevelID").ToString)/UnitRatioVal), "##,##0.00") + "</a></td>")   
						End If
						Summary(j) += displayTable.Rows(i)("Shop_" + ShopData.Rows(j)("ProductLevelID").ToString)
				   Next 
				   If UnitRatioVal = 1 Or displayTable.Rows(i)("SummaryAmount") = 0 Then
				   		outputString = outputString.Append("<td align=""right"" class=""" + TextClass + """>" + Format(Math.ABS(displayTable.Rows(i)("SummaryAmount")/UnitRatioVal), "##,##0") + "</td>") 
				   Else
				   		outputString = outputString.Append("<td align=""right"" class=""" + TextClass + """>" + Format(Math.ABS(displayTable.Rows(i)("SummaryAmount")/UnitRatioVal), "##,##0.00") + "</td>") 
				   End If   
			    ...
			    'For j = 0 To ShopData.Rows.Count - 1 
					'outputString = outputString.Append("<td align=""right"" class=""" + TextClass + """>" + Format(Math.ABS(Summary(j)/UnitRatioVal), "##,##0") + "</td>")    
			    'Next   
			    'outputString = outputString.Append("<td align=""right"" class=""" + TextClass + """>" + Format(Math.ABS(SumEach/UnitRatioVal), "##,##0") + "</td>")       
			    'outputString = outputString.Append("</tr>")  
...
Public Function ReportProductList(ByVal ListID As String, ByVal objCnn As MySqlConnection) As DataTable
	Dim dtTable As DataTable
	Dim AdditionalQuery As String = ""
	If Trim(ListID) <> "" Then AdditionalQuery += " AND ListID IN (" + ListID + ")"
	dtTable = objDB.List("select * from Report_Product_List where 0=0 " + AdditionalQuery + " order by Ordering", objCnn)
	return dtTable
End Function
