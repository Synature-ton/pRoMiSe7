...
<html>
<head>
<title>Company Sales Payment Report</title>
...
<tr bgcolor="eeeeee">
	...
	<td colspan="2" background="../images/headerbg2000.jpg">
		<div class="noprint"><table width="100%"><tr><td align="left" width="70%"><b class="headerText"><asp:Label ID="LangText0" Text="Company Sales Report" runat="server" /></b></td><td align="right"><asp:Label ID="LangList" runat="server" /></td></tr></table></div>
	</td>
...
Sub Page_Load()
	If User.Identity.IsAuthenticated  AND Session("Report_CompanySalesPayment") Then
	...
	'Try	
		...
		LangText0.Text = "Company Sales Payment Report"'LangData2.Rows(16)(LangText)
		...
		Dim ShopData As DataTable = objDB.List("select distinct(CompanyCode) As CompanyCode from ShopExtData order by CompanyCode", objCnn)'getInfo.GetProductLevelAccess(-999,Session("StaffRole"),objCnn)
		If ShopData.Rows.Count > 0 Then
			...
			If ShopData.Rows.Count > 1 Then
				If Not Page.IsPostBack Then 
					...
					SelShopName.Value = "All Companies"
				ElseIf ShopIDValue = "" Then
					FormSelected = "selected"
					SelShopName.Value = "All Companies"
				Else
					FormSelected = ""
				End If
				outputString += "<option value=""" & "" & """ " & FormSelected & ">" & "--- All Companies ---"
			End If
			For i = 0 to ShopData.Rows.Count - 1
				If ShopIDValue = ShopData.Rows(i)("CompanyCode") Then
					FormSelected = "selected"
					SelShopName.Value = ShopData.Rows(i)("CompanyCode")
				Else
					FormSelected = ""
				End If
				outputString += "<option value=""" & ShopData.Rows(i)("CompanyCode") & """ " & FormSelected & ">" & ShopData.Rows(i)("CompanyCode")
				ShopList += "," + ShopData.Rows(i)("CompanyCode").ToString
			Next
			outputString += "</select>"
...
Sub DoSearch(Source As Object, E As EventArgs)
	...
	If FoundError = False Then
		...
		Dim dtTable As DataTable = CompanySalesPaymentReport(PaymentData, StartDate, EndDate, Request.Form("ShopID"), objCnn)
		
		Dim PayTypeList As DataTable = PaymentData.DefaultView.ToTable(True, "PayTypeID","PayTypeName")
		
		Dim HeaderString As String = ""
		
		HeaderString += "<td align=""center"" class=""smallTdHeader"" bgcolor=""" + GlobalParam.AdminBGColor + """>" + "CV" + "</td>"

		HeaderString += "<td align=""center"" class=""smallTdHeader"" bgcolor=""" + GlobalParam.AdminBGColor + """>" + "Store Code" + "</td>"
		HeaderString += "<td align=""center"" class=""smallTdHeader"" bgcolor=""" + GlobalParam.AdminBGColor + """>" + "Store Name" + "</td>"
		HeaderString += "<td align=""center"" class=""smallTdHeader"" bgcolor=""" + GlobalParam.AdminBGColor + """>" + "Date" + "</td>"
		HeaderString += "<td align=""center"" class=""smallTdHeader"" bgcolor=""" + GlobalParam.AdminBGColor + """>" + "Sales Total" + "</td>"
		...
		For i = 0 To dtTable.Rows.Count - 1
			...
			If Not IsDBNull(dtTable.Rows(i)("CompanyCode")) Then
           	 	outputString = outputString.Append("<td align=""left"" class=""" + TextClass + """>" & dtTable.Rows(i)("CompanyCode") & "</td>")
			Else
				outputString = outputString.Append("<td align=""left"" class=""" + TextClass + """>" & "" & "</td>")
			End If
			If Not IsDBNull(dtTable.Rows(i)("ERPShopCode")) Then
           	 	outputString = outputString.Append("<td align=""left"" class=""" + TextClass + """>" & dtTable.Rows(i)("ERPShopCode") & "</td>")
			Else
				outputString = outputString.Append("<td align=""left"" class=""" + TextClass + """>" & "" & "</td>")
			End If
			If Not IsDBNull(dtTable.Rows(i)("ShopName")) Then
           	 	outputString = outputString.Append("<td align=""left"" class=""" + TextClass + """>" & dtTable.Rows(i)("ShopName") & "</td>")
			Else
				outputString = outputString.Append("<td align=""left"" class=""" + TextClass + """>" & "" & "</td>")
			End If
			outputString = outputString.Append("<td align=""right"" class=""" + TextClass + """>" + DateTimeUtil.FormatDateTime(dtTable.Rows(i)("SaleDate"), "DateOnly", Session("LangID"), objCnn) + "</td>")
			If Not IsDBNull(dtTable.Rows(i)("TotalSalePrice")) Then
           	 	outputString = outputString.Append("<td align=""right"" class=""" + TextClass + """>" + CDbl(dtTable.Rows(i)("TotalSalePrice")).ToString(FormatObject.CurrencyFormat, ci) + "</td>")
			Else
				outputString = outputString.Append("<td align=""left"" class=""" + TextClass + """>" & "" & "</td>")
			End If
			For j = 0 To PayTypeList.Rows.Count - 1
				expression = "PayTypeID=" + PayTypeList.Rows(j)("PayTypeID").ToString + " AND ShopID=" + dtTable.Rows(i)("ShopID").ToString + " AND DateString='" + dtTable.Rows(i)("DateString") + "'"
		...
		If Request.Form("ShopID") = "" Then
			ShopDisplay = "All Companies"
		Else
			ShopDisplay = SelShopName.Value
		End If
		ResultSearchText.InnerHtml = "Sales Payment Report for Company" + " " + ShopDisplay + " (" + ReportDate + ")"
...
Sub ExportData(Source As Object, E As EventArgs)
	...
	Dim FileName As String = "CompanySalesPaymentData_" & DateTime.Now.ToString("yyyyMMdd_HHmmss", InvC) & ".xls"
...
Public Function CompanySalesPaymentReport(ByRef PaymentData As DataTable, ByVal StartDate As String, ByVal EndDate As String, ByVal CompanyCode As String, ByVal objCnn As MySqlConnection) As DataTable
        ...
        If CompanyCode <> "" Then
            AdditionalQuery += " AND b.CompanyCode='" + Replace(CompanyCode,"'","''") + "'"
        End If
        ...
        sqlStatement = "select a.ShopID,b.ERPShopCode,b.CompanyCode,a.ShopName,a.SaleDate,SUM(SalePrice) As TotalSalePrice,DATE_FORMAT(a.SaleDate,'%d%m%Y') As DateString from summary_tranreport a left outer join ShopExtData b ON a.ShopID=b.ShopID where 0=0 " + AdditionalQuery + " group by a.ShopID,b.ERPShopCode,b.CompanyCode,a.ShopName,a.SaleDate order by b.CompanyCode,a.ShopID,a.SaleDate"

        GetData = objDB.List(sqlStatement, objCnn)
		
		sqlStatement = "select a.ShopID,a.SaleDate,a.PayTypeID,a.PayTypeName,SUM(TotalPay) As TotalPay,DATE_FORMAT(a.SaleDate,'%d%m%Y') As DateString from summary_paymentsolariareport a left outer join ShopExtData b ON a.ShopID=b.ShopID where 0=0 " + AdditionalQuery + " group by a.ShopID,a.SaleDate,a.PayTypeID,a.PayTypeName order by a.PayTypeID"
