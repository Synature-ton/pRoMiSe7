...
Sub DoSearch(Source As Object, E As EventArgs)
	...
	If FoundError = False Then
		...
		ResultText.InnerHtml = PriceRangeReport(dtTable, StartDate, EndDate, Request.Form("ShopID"), LangPath, Session("StaffRole"),receiptSaleType, objCnn)
		...
		ResultSearchText.InnerHtml = "Price Range Report" + " " + ShopDisplay + " (" + ReportDate + ")" & strDisplaySaleType
		'Application.UnLock()
...
End Sub	
 ...
 Public Function PriceRangeReport(ByRef dtTable As DataTable, ByVal StartDate As String, ByVal EndDate As String, ByVal ShopID As String, ByVal LangPath As String, ByVal StaffRoleID As Integer, receiptSaleNonSaleType as integer, ByVal objCnn As MySqlConnection) As String
		...
		Select Case receiptSaleNonSaleType
			Case 2				'Non Sale
				AdditionalQuery += " AND TransactionStatusID = 11 AND DocType NOT IN (4,8) "
			Case 3				'Sale + Non Sale
				AdditionalQuery += " AND TransactionStatusID IN (2,11) "
			Case Else			'Sale
				AdditionalQuery += " AND TransactionStatusID = 2 AND DocType IN (4,8) "
		End Select
		
        sqlStatement = "select StartValue,EndValue,count(CASE WHEN ReceiptSalePrice between StartValue and EndValue THEN 1 END) As TotalBill from Report_RangeSales a cross join ordertransaction b where ReceiptID>0 " + AdditionalQuery + " group by StartValue,EndValue order by StartValue"

		dtTable = objDb.List(sqlStatement, objCnn)
		
		Dim MaxValue As Double = 0
		Dim chkMax As DataTable = objDB.List("select MAX(ReceiptSalePrice) As MaxSalePrice from ordertransaction where ReceiptID>0 " + AdditionalQuery, objCnn)
