...
Sub DoSearch(Source As Object, E As EventArgs)
	...
	If FoundError = False Then
		...
		End Select 
            ...
            ResultSearchText.InnerHtml = ReportType + " " + ShopDisplay + " (" + ReportDate + ")" & strDisplaySaleType
            
            strTime &= ", EndTime : " & Format(Now, "HH:mm:ss")
            'errorMsg.InnerHtml = strTime
...
Public Function SaleMixReports(ByRef HourlyData As DataTable, ByRef IncomeType As DataTable, ByRef PayTypeList As DataTable, ByRef outputString As String, ByRef grandTotal As Double, ByRef VATTotal As Double, ByRef GraphData As DataSet, ByVal ShowSummary As Boolean, ByVal GrayBGColor As String, ByVal AdminBGColor As String, ByVal LangID As Integer, ByVal ViewOption As Integer, ByVal ReportByBill As Boolean, ByVal ReportByProduct As Boolean, ByVal ReportByTime As Boolean, ByVal GroupByParam As Integer, ByVal StartDate As String, ByVal EndDate As String, ByVal StartTimeHour As String, ByVal StartTimeMinute As String, ByVal EndTimeHour As String, ByVal EndTimeMinute As String, ByVal ShopID As String, ByVal TransactionID As Integer, ByVal ComputerID As Integer, ByVal DisplaySummary As Boolean, ByVal ExpandReceipt As Boolean, ByVal DisplayGraph As Boolean, ByVal LangPath As String, ByVal BySaleMode As Boolean, ByVal receiptSaleNonSaleType As Integer, ByVal StaffRoleID As Integer, ByVal objCnn As MySqlConnection) As String
        ...
        If ReportByProduct = True Or ReportByBill = True Then
        	...
        	Else
				' Before adding Sale/ NonSale Receipt Type --> SaleMix = ByProduct=True, ByReceipt = False, Data From Summary_ProductReport
				'											   OC = ByProduct = False, ByReceipt = True, Data From Summary_ProductReport_StockOnly
				' SaleMix = ByProduct For DocType = 8, OC = ByProduct For StockOnly
				'When Adding Sale/ NonSale Receipt Type ---> SaleMix/ OC report will be the same (Only No Display Summary When Select OC)
				...
				End If
				 Select Case ReceiptSaleNonSaleType
					...
					Case 3          'Sale + Non Sale --> No Specific Document
			...
			Case 2				'Non Sale
				ReportTable = "summary_productreport_stockonly"
				sqlStatement = "SELECT IF(a.ProductLinkID is NULL,-99,a.ProductLinkID) As ProductIDLink,a.TransactionStatusID,a.OtherFoodName,a.OrderStatusID,a.ProductSetType,a.VATType, p.ProductCode," + PName + ", a.ProductDeptName,a.ProductGroupName,a.ProductGroupCode,a.ProductDeptCode,a.GroupOrdering,a.DeptOrdering,a.SaleMode,sm.SaleModeName,sm.PositionPrefix,sm.PrefixText,SUM(a.Amount) AS Amount,SUM(a.TotalPrice) AS TotalPrice,SUM(a.TotalRetailPrice) AS TotalRetailPrice,SUM(SalePrice) AS SalePrice FROM " + ReportTable + " a left outer join Products p ON a.ProductID=p.ProductID left outer join SaleMode sm ON a.SaleMode=sm.SaleModeID WHERE a.OrderStatusID IN (1,2,5) " + AdditionalQuery & strTransStatus & "  GROUP BY a.ProductLinkID,a.TransactionStatusID,a.OtherFoodName,a.OrderStatusID,a.ProductSetType,a.VATType, p.ProductCode,p.ProductName, a.ProductDeptName,a.ProductGroupName,a.ProductGroupCode,a.ProductDeptCode,a.SaleMode,sm.SaleModeName,sm.PositionPrefix,sm.PrefixText ORDER BY " + OrderingText
		
                Case 3              'Sale + Non Sale
                    Dim bolUseCombineTable As Boolean = False
                    'bolUseCombineTable = getProp.CheckTableExist("Summary_ProductReport_CombineSaleNonSale", objCnn)
                    bolUseCombineTable = False          'From Testing, Using Pre-Gen Table is slower than create temp table DummyAllProductSaleMix
                    If bolUseCombineTable = False Then
                        sqlStatement = "DROP TABLE IF EXISTS DummyAllProductSaleMix_" + ExtraTableString
                        objDB.sqlExecute(sqlStatement, objCnn)
                        sqlStatement = "create table DummyAllProductSaleMix_" & ExtraTableString & " (ProductLinkID int, TransactionStatusID smallint,OtherFoodName varchar(100), " & _
                                    "OrderStatusID tinyint, ProductSetType tinyint, VATType tinyint, DocType int, ProductID int, " & _
                                    "ProductDeptName varchar(100),ProductGroupName varchar(100), " & _
                                    "ProductGroupCode varchar(50), ProductDeptCode varchar(50), GroupOrdering int, DeptOrdering int, SaleMode tinyint, Amount decimal(18,4), " & _
                                    "TotalPrice decimal(18,4), TotalRetailPrice decimal(18,4), SalePrice decimal(18,4))"
                        objDB.sqlExecute(sqlStatement, objCnn)
                        'Insert From Summary_ProductReport
                        sqlStatement = "Insert INTO DummyAllProductSaleMix_" & ExtraTableString & "(ProductLinkID, TransactionStatusID, OtherFoodName, OrderStatusID, " & _
                                     "ProductSetType, VATType, DocType, ProductID, ProductDeptName, ProductGroupName, ProductDeptCode, ProductGroupCode, " & _
                                     "GroupOrdering, DeptOrdering, SaleMode, Amount, TotalPrice, TotalRetailPrice, SalePrice) " & _
                                     "SELECT ProductLinkID,TransactionStatusID, OtherFoodName,OrderStatusID,ProductSetType,VATType, DocType, ProductID, " & _
                                     "ProductDeptName,ProductGroupName, ProductDeptCode, ProductGroupCode, GroupOrdering,DeptOrdering,SaleMode,Amount,TotalPrice, " & _
                                     "TotalRetailPrice,SalePrice " & _
                                     "FROM summary_productreport a " & _
                                     "WHERE a.OrderStatusID IN (1,2,5) " & AdditionalQuery
                        objDB.sqlExecute(sqlStatement, objCnn)
                        'Insert From Summary_ProductReport_StockOnly
                        sqlStatement = "Insert INTO DummyAllProductSaleMix_" & ExtraTableString & "(ProductLinkID, TransactionStatusID, OtherFoodName, OrderStatusID, " & _
                                     "ProductSetType, VATType, DocType, ProductID, ProductDeptName, ProductGroupName, ProductDeptCode, ProductGroupCode, " & _
                                     "GroupOrdering, DeptOrdering, SaleMode, Amount, TotalPrice, TotalRetailPrice, SalePrice) " & _
                                     "SELECT ProductLinkID,TransactionStatusID, OtherFoodName,OrderStatusID,ProductSetType,VATType, DocType, ProductID, " & _
                                     "ProductDeptName,ProductGroupName, ProductDeptCode, ProductGroupCode, GroupOrdering,DeptOrdering,SaleMode,Amount,TotalPrice, " & _
                                     "TotalRetailPrice,SalePrice " & _
                                     "FROM summary_productreport_StockOnly a " & _
                                     "WHERE a.OrderStatusID IN (1,2,5) " & AdditionalQuery
                        objDB.sqlExecute(sqlStatement, objCnn)
					
                        sqlStatement = "SELECT IF(a.ProductLinkID is NULL,-99,a.ProductLinkID) As ProductIDLink,Min(a.TransactionStatusID) as TransactionStatusID,a.OtherFoodName,a.OrderStatusID, " & _
                                "a.ProductSetType,a.VATType, p.ProductCode," + PName + ", a.ProductDeptName,a.ProductGroupName,a.ProductGroupCode,a.ProductDeptCode, " & _
                                "Min(a.GroupOrdering) as GroupOrdering,Min(a.DeptOrdering) as DeptOrdering,a.SaleMode,sm.SaleModeName,sm.PositionPrefix,sm.PrefixText,SUM(a.Amount) AS Amount, " & _
                                " SUM(a.TotalPrice) AS TotalPrice,SUM(a.TotalRetailPrice) AS TotalRetailPrice,SUM(SalePrice) AS SalePrice " & _
                                "FROM DummyAllProductSaleMix_" & ExtraTableString & " a LEFT OUTER JOIN Products p ON a.ProductID=p.ProductID " & _
                                "  left outer join SaleMode sm ON a.SaleMode=sm.SaleModeID " & _
                                "  GROUP BY a.ProductLinkID,a.OtherFoodName,a.OrderStatusID,a.ProductSetType,a.VATType, " & _
                                " p.ProductCode,p.ProductName, a.ProductDeptName,a.ProductGroupName,a.ProductGroupCode,a.ProductDeptCode,a.SaleMode,sm.SaleModeName, " & _
                                " sm.PositionPrefix,sm.PrefixText ORDER BY " + OrderingText
                    Else
                        sqlStatement = "SELECT IF(a.ProductLinkID is NULL,-99,a.ProductLinkID) As ProductIDLink,Min(a.TransactionStatusID) as TransactionStatusID,a.OtherFoodName,a.OrderStatusID, " & _
                                "a.ProductSetType,a.VATType, p.ProductCode," + PName + ", a.ProductDeptName,a.ProductGroupName,a.ProductGroupCode,a.ProductDeptCode, " & _
                                "Min(a.GroupOrdering) as GroupOrdering,Min(a.DeptOrdering) as DeptOrdering,a.SaleMode,sm.SaleModeName,sm.PositionPrefix,sm.PrefixText,SUM(a.Amount) AS Amount, " & _
                                " SUM(a.TotalPrice) AS TotalPrice,SUM(a.TotalRetailPrice) AS TotalRetailPrice,SUM(SalePrice) AS SalePrice " & _
                                "FROM Summary_ProductReport_CombineSaleNonSale a LEFT OUTER JOIN Products p ON a.ProductID=p.ProductID " & _
                                "  left outer join SaleMode sm ON a.SaleMode=sm.SaleModeID " & _
                                "WHERE a.OrderStatusID IN (1,2,5) " + AdditionalQuery & strTransStatus & _
                                "  GROUP BY a.ProductLinkID,a.OtherFoodName,a.OrderStatusID,a.ProductSetType,a.VATType, " & _
                                " p.ProductCode,p.ProductName, a.ProductDeptName,a.ProductGroupName,a.ProductGroupCode,a.ProductDeptCode,a.SaleMode,sm.SaleModeName, " & _
                                " sm.PositionPrefix,sm.PrefixText ORDER BY " + OrderingText
                    End If
			...
			Dim ProductData As DataTable = objDB.List("select ProductCode,ProductName,ProductSetType,OrderStatusID,ProductIDLink,VATType,ProductGroupCode,ProductGroupName,ProductDeptCode,ProductDeptName,Min(GroupOrdering) as GroupOrdering,Min(DeptOrdering) as DeptOrdering,count(*) from  DummySaleMix_" + ExtraTableString + " group by ProductCode,ProductName,ProductSetType,OrderStatusID,ProductIDLink,VATType,ProductGroupCode,ProductGroupName,ProductDeptCode,ProductDeptName order by GroupOrdering,DeptOrdering,ProductCode", objCnn)
            ...
            SaleMixReportByProduct(BySaleMode, SMData, PromotionData, ProductData, GraphData, outputString, grandTotal, VATTotal, ShowSummary, GrayBGColor, AdminBGColor, ShopIDListValue, ViewOption, StartDate, EndDate, GetData, PaymentData, PayByCreditMoney,receiptSaleNonSaleType, LangID, LangPath, objCnn)
            TimeNow += "<BR>" + DateTimeUtil.CurrentDateTime		
        ...
        Else
			...
			Dim getOpenHour As DataTable = objDB.List("select HOUR(OpenHour) As OpenHour,HOUR(CloseHour) As CloseHour from productlevel where ProductLevelID IN (" + ShopID + ") Order By ProductLevelID", objCnn)
			...
			Dim ProductData As DataTable = objDB.List("select ProductCode,ProductName,ProductSetType,OrderStatusID,ProductIDLink,VATType,ProductGroupCode,ProductGroupName,ProductDeptCode,ProductDeptName,Min(GroupOrdering) as GroupOrdering,Min(DeptOrdering) as DeptOrdering,count(*) from  DummySaleMixHourly_" + ExtraTableString + " group by ProductCode,ProductName,ProductSetType,OrderStatusID,ProductIDLink,VATType,ProductGroupCode,ProductGroupName,ProductDeptCode,ProductDeptName order by GroupOrdering,DeptOrdering,ProductCode", objCnn)
			...
			SaleMixHourlyReportByProduct(BySaleMode, HourlyData, PromotionData, ProductData, GraphData, outputString, grandTotal, VATTotal, ShowSummary, GrayBGColor, AdminBGColor, ShopIDListValue, ViewOption, StartDate, EndDate, GetData, PaymentData, PayByCreditMoney, receiptSaleNonSaleType, LangID, LangPath, objCnn)
            TimeNow += "<BR>" + DateTimeUtil.CurrentDateTime
    ...
    Public Function SaleMixReportByProduct(ByVal BySaleMode As Boolean, ByVal SMData As DataTable, ByVal PromotionData As DataTable, ByVal ProductData As DataTable, ByRef GraphData As DataSet, ByRef ResultString As String, ByRef grandTotal As Double, ByRef VATTotal As Double, ByVal ShowSummary As Boolean, ByVal GrayBGColor As String, ByVal AdminBGColor As String, ByVal ShopID As String, ByVal ViewOption As Integer, ByVal StartDate As String, ByVal EndDate As String, ByVal dtTable As DataTable, ByVal PaymentResult As DataTable, ByVal PayByCreditMoney As DataTable, receiptSaleNonSaleType As Integer, ByVal LangID As Integer, ByVal LangPath As String, ByVal objCnn As MySqlConnection) As String
           ...
           ' ElseIf Not IsDBNull(ProductData.Rows(i)("OtherFoodName")) Then
			...
			For j = 0 To SMData.Rows.Count - 1
				If ProductData.Rows(i)("ProductName") = "" then
					strProductName = ""
					expression = "ProductCode IS NULL AND ProductSetType=" + ProductData.Rows(i)("ProductSetType").ToString + " AND SaleMode=" + SMData.Rows(j)("SaleModeID").ToString + " AND VATType=" + ProductData.Rows(i)("VATType").ToString + " AND OrderStatusID=" + ProductData.Rows(i)("OrderStatusID").ToString + " AND ProductIDLink=" + ProductData.Rows(i)("ProductIDLink").ToString + " AND ProductName IS NULL"
				else
					strProductName = Replace(ProductData.Rows(i)("ProductName"),"'","''").ToString
					expression = "ProductCode='" + ProductData.Rows(i)("ProductCode").ToString + "' AND ProductSetType=" + ProductData.Rows(i)("ProductSetType").ToString + " AND SaleMode=" + SMData.Rows(j)("SaleModeID").ToString + " AND VATType=" + ProductData.Rows(i)("VATType").ToString + " AND OrderStatusID=" + ProductData.Rows(i)("OrderStatusID").ToString + " AND ProductIDLink=" + ProductData.Rows(i)("ProductIDLink").ToString + " AND ProductName='" + strProductName + "'"
				...
				For k = 0 To foundRows.GetUpperBound(0) 
					ProductQty += foundRows(k)("Amount")
					subTotalQty += ProductQty
                    grandTotalQty += ProductQty
                    subTotalGroupQty += ProductQty
					SaleModeQty += ProductQty
					TotalRetailPrice += foundRows(k)("TotalRetailPrice")
					TotalSale += foundRows(k)("SalePrice")
					TotalDiscount += foundRows(k)("TotalRetailPrice") - foundRows(k)("SalePrice")
					
					subTotalRetailPrice += foundRows(k)("TotalRetailPrice")
					subTotalSale += foundRows(k)("SalePrice")
					subTotalDiscount += foundRows(k)("TotalRetailPrice") - foundRows(k)("SalePrice")
					
					subTotalGroupRetailPrice += foundRows(k)("TotalRetailPrice")
					subTotalGroupSale += foundRows(k)("SalePrice")
					subTotalGroupDiscount += foundRows(k)("TotalRetailPrice") - foundRows(k)("SalePrice")
					
					grandTotalRetailPrice += foundRows(k)("TotalRetailPrice")
					grandTotalSale += foundRows(k)("SalePrice")
					grandTotalDiscount += foundRows(k)("TotalRetailPrice") - foundRows(k)("SalePrice")
				Next k
				
'				If foundRows.GetUpperBound(0) >= 1 Then
'				outputString = outputString.Append("<td align=""center"" class=""" + TextClass + """>Row = " & foundRows.GetUpperBound(0) & "</td>")
'				End If
...
'            Dim dtServiceChargeAndVAT As DataTable = objDB.List("select SUM(a.ServiceCharge) AS ServiceCharge, SUM(a.ServiceChargeVAT) AS ServiceChargeVAT, SUM(a.TransactionVAT) AS TotalVAT, Sum(a.TotalBill) as TotalBill from OrderTransaction a WHERE ReceiptID > 0 AND SaleDate >= " + StartDate + " AND SaleDate < " & EndDate & strTransStatus & ServiceChargeQuery, objCnn)

            Dim dtServiceChargeAndVAT As DataTable = objDB.List("select SUM(a.ServiceCharge) AS ServiceCharge, SUM(a.ServiceChargeVAT) AS ServiceChargeVAT, SUM(a.TransactionVAT) AS TotalVAT, Sum(a.TotalBill) as TotalBill from Summary_TranReport a WHERE SaleDate >= " + StartDate + " AND SaleDate < " & EndDate & strTransStatus & ServiceChargeQuery, objCnn)
						
            If Not IsDBNull(dtServiceChargeAndVAT.Rows(0)("ServiceCharge")) Then
                grandTotalServiceCharge = dtServiceChargeAndVAT.Rows(0)("ServiceCharge") + dtServiceChargeAndVAT.Rows(0)("ServiceChargeVAT")
                grandTotalServiceChargeVAT = dtServiceChargeAndVAT.Rows(0)("ServiceChargeVAT")
            Else
            ...
            Else
                ...
                If Not IsDBNull(dtServiceChargeAndVAT.Rows(0)("TotalVAT")) Then
                    outputString = outputString.Append("<td align=""right"" class=""smallText"">" + CDbl(dtServiceChargeAndVAT.Rows(0)("TotalVAT")).ToString(FormatObject.CurrencyFormat, ci) + "</td><td>&nbsp;</td><td>&nbsp;</td></tr>")
                    grandTotalSaleAvg += grandTotalServiceCharge + grandTotalOtherIncome
                    grandTotalAfterVAT += dtServiceChargeAndVAT.Rows(0)("TotalVAT")
                Else
	...
	Public Function SaleMixHourlyReportByProduct(ByVal BySaleMode As Boolean, ByVal HourlyData As DataTable, ByVal PromotionData As DataTable, ByVal ProductData As DataTable, ByRef GraphData As DataSet, ByRef ResultString As String, ByRef grandTotal As Double, ByRef VATTotal As Double, ByVal ShowSummary As Boolean, ByVal GrayBGColor As String, ByVal AdminBGColor As String, ByVal ShopID As String, ByVal ViewOption As Integer, ByVal StartDate As String, ByVal EndDate As String, ByVal dtTable As DataTable, ByVal PaymentResult As DataTable, ByVal PayByCreditMoney As DataTable, receiptSaleNonSaleType As Integer, ByVal LangID As Integer, ByVal LangPath As String, ByVal objCnn As MySqlConnection) As String
		...
		For i = 0 To ProductData.Rows.Count - 1
			...
			For j = 0 To HourlyData.Rows.Count - 1
				...
				End If
				expression = "ProductCode='" + ProductData.Rows(i)("ProductCode").ToString + "' AND ProductSetType=" + ProductData.Rows(i)("ProductSetType").ToString + " AND VATType=" + ProductData.Rows(i)("VATType").ToString + " AND OrderStatusID=" + ProductData.Rows(i)("OrderStatusID").ToString + " AND ProductIDLink=" + ProductData.Rows(i)("ProductIDLink").ToString + " AND ProductName='" + Replace(ProductData.Rows(i)("ProductName"),"'","''").ToString + "'" + AdditionalQuery
        ...
        If ShowSummary = True Then
            ...
            Dim ProductLevelID As String = ShopID
            If ShopID = "0" Then ProductLevelID = "1"
            Dim ShopInfo As DataTable = objDB.List("select * from ProductLevel where ProductLevelID IN (" + ProductLevelID + ")", objCnn)
            Dim DisplayVATType As String = "V"
            If ShopInfo.Rows.Count > 0 Then
                If ShopInfo.Rows(0)("DisplayReceiptVATableType") = 2 Then
                    DisplayVATType = "E"
            ...
            Dim serviceCharge As DataTable = objDB.List("select SUM(a.ServiceCharge) AS ServiceCharge, SUM(a.ServiceChargeVAT) AS ServiceChargeVAT from  ordertransaction a WHERE ReceiptID > 0 AND SaleDate >= " + StartDate + " AND SaleDate < " & EndDate & strTransStatus & ServiceChargeQuery, objCnn)
            ...
            If DisplayVATType = "V" Then
                ...
                outputString = outputString.Append("<tr bgColor=""" + GrayBGColor + """>")
                outputString = outputString.Append("<td colspan=""" + ColSpan3 + """ align=""right"" class=""" + TextClass + """>" + LangData2.Rows(95)(LangText) + "</td>")
                outputString = outputString.Append("<td align=""right"" class=""smallText"">" + CDbl(grandTotalAfterVAT).ToString(FormatObject.CurrencyFormat, ci) + "TY</td><td>&nbsp;</td><td>&nbsp;</td></tr>")
            ...
            Else
                ...
                Dim VATData As DataTable = objDB.List("select SUM(a.TransactionVAT) AS TotalVAT from  ordertransaction a WHERE ReceiptID > 0 AND SaleDate >= " + StartDate + " AND SaleDate < " & EndDate & strTransStatus & ServiceChargeQuery, objCnn)
                outputString = outputString.Append("<tr bgColor=""" + GrayBGColor + """>")
                outputString = outputString.Append("<td colspan=""" + ColSpan3 + """ align=""right"" class=""" + TextClass + """>" + "Tax" + "</td>")
                If Not IsDBNull(VATData.Rows(0)("TotalVAT")) Then
                    ...
                    grandTotalSaleAvg += grandTotalServiceCharge + grandTotalOtherIncome
                    grandTotalAfterVAT += grandTotalServiceCharge + VATData.Rows(0)("TotalVAT") - grandTotalServiceChargeVAT
                Else
                    outputString = outputString.Append("<td align=""right"" class=""smallText"">" + CDbl(0).ToString(FormatObject.CurrencyFormat, ci) + "</td><td>&nbsp;</td><td>&nbsp;</td></tr>")
                End If
                outputString = outputString.Append("<tr bgColor=""" + GrayBGColor + """>")
