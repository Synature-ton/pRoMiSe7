...
<table border="1" cellpadding="4" cellspacing="0" style="border-collapse:collapse;" width="100%">
	<tr>
		...
		<td id="headerTD2" align="center" class="tdHeader" runat="server"><div id="NameText" runat="server"></div><asp:LinkButton ID="NameAsc" Text="Asc" CssClass="tableHeader" OnClick="DoSearch" runat="server" />&nbsp;&nbsp;&nbsp;&nbsp;<asp:LinkButton ID="NameDesc" Text="Desc" CssClass="tableHeader" OnClick="DoSearch" runat="server" /></td>
		<td id="headerTD11" align="center" class="tdHeader" runat="server"><div id="CMReceiptText" runat="server"></div></td>
		<td id="headerTD3" align="center" class="tdHeader" runat="server"><div id="ReceiptText" runat="server"></div><asp:LinkButton ID="OrderAsc" Text="Asc" CssClass="tableHeader" OnClick="DoSearch" runat="server" />&nbsp;&nbsp;&nbsp;&nbsp;<asp:LinkButton ID="OrderDesc" Text="Desc" CssClass="tableHeader" OnClick="DoSearch" runat="server" /></td>
		...
		<td id="headerTD4" align="center" class="tdHeader" runat="server"><div id="DateText" runat="server"></div><asp:LinkButton ID="DateAsc" Text="Asc" CssClass="tableHeader" OnClick="DoSearch" runat="server" />&nbsp;&nbsp;&nbsp;&nbsp;<asp:LinkButton ID="DateDesc" Text="Desc" CssClass="tableHeader" OnClick="DoSearch" runat="server" /></td>
...
Sub Page_Load()
	If User.Identity.IsAuthenticated AND (Session("Member_CreditMoney") Or Session("Report_Sale_New")) Then
		...
		'Try
			...
			GetCreditMoneyReport(Request.QueryString("ViewOption"),Request.QueryString("StartDate"),Request.QueryString("EndDate"),Request.QueryString("ShopID"),Request.QueryString("PayTypeID"),Session("LangID"),objCnn)
...
Public Function GetCreditMoneyReport(ByVal ViewOption As Integer, ByVal DateFrom As String, ByVal DateTo As String, ByVal ShopID As Integer, ByVal PayTypeID As Integer, ByVal LangID As Integer, ByVal objCnn As MySqlConnection) As DataTable
	...
	If FoundError = False Then
		...
		displayTable = getReport.CreditMoneyReport(ViewOption,DateFrom,DateTo,ShopID,PayTypeID,LangID,OrderByParam,objCnn)
		...
		For i=0 To displayTable.Rows.Count - 1
			...
			End If
			outputString += "<td align=""left"" class=""text""><a href=""JavaScript: newWindow = window.open( '../Reports/BillDetails.aspx?ComputerID=" + displayTable.Rows(i)("ComputerID").ToString + "&TransactionID=" + displayTable.Rows(i)("TransactionID").ToString + "&ShopID=" + ShopID.ToString + "', '', 'width=750,height=600,toolbar=0,location=0,directories=0,status=0,menuBar=0,scrollBars=1,resizable=1' ); newWindow.focus()"">" & ReceiptHeader & "</a></td>"
		...
		Else
			outputString += "<tr bgcolor=""" + GlobalParam.GrayBGColor + """><td colspan=""2"" class=""text"" align=""right"">Total Amount</td><td align=""right"" class=""text"">" + Format(grandTotalThisPeriod,"##,##0.00") + "</td><td></td></tr>"
		End If
