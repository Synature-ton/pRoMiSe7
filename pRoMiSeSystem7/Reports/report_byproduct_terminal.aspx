...
<html>
<head>
<title>Sale Product By Terminal</title>
...
<tr bgcolor="eeeeee">
	...
	<td colspan="2" background="../images/headerbg2000.jpg">
		<div class="noprint"><table width="100%"><tr><td align="left" width="70%"><b class="headerText"><asp:Label ID="LangText0" Text="Product Sale By Terminal Report" runat="server" /></b></td><td align="right"><asp:Label ID="LangList" Visible="false" runat="server" /></td></tr></table></div>
	</td>
...
<tr id="ShowShop" runat="server">
	...
	<table>
		<tr>
			...
			<td colspan="4"><table cellpadding="0" cellspacing="0"><tr><td><synature:date id="DailyDate" runat="server" /></td><td>&nbsp;&nbsp;</td><td>&nbsp;</td></tr></table></td>
		</tr>
		<tr>
...
Sub Page_Load()
	If User.Identity.IsAuthenticated  AND Session("ProductSaleByTerminal") Then
	...
	Try	
		...
		LangText0.Text = "Product Sale By Terminal Report"
...
Sub DoSearch(Source As Object, E As EventArgs)
	...
	If FoundError = False Then
		...
		Dim dtTable As DataTable = SaleProductTerminal(PayTypeList,PayTypeData,StartDate, EndDate, Request.Form("ShopID"), Session("LangID"), ViewOption, objCnn)
		ResultSearchText.InnerHtml = "Product Sale by Terminal Report" + "<BR>" + SelShopName.Value + " " + ReportDate
		'Application.UnLock()
		...
		HeaderString += "<td align=""center"" class=""smallTdHeader"" bgcolor=""" + GlobalParam.AdminBGColor + """>Barcode</td>"
		...
		HeaderString += "<td align=""center"" class=""smallTdHeader"" bgcolor=""" + GlobalParam.AdminBGColor + """>Retail Price</td>"
		
		HeaderString += "<td align=""center"" class=""smallTdHeader"" bgcolor=""" + GlobalParam.AdminBGColor + """>Discount</td>"
		HeaderString += "<td align=""center"" class=""smallTdHeader"" bgcolor=""" + GlobalParam.AdminBGColor + """>Sale Price</td>"
		...
		Dim POSTotalQty As Double = 0
		Dim POSTotalVAT As Double = 0
		Dim POSTotalSale As Double = 0
		...
		For i = 0 To dtTable.Rows.Count - 1
			...
			grandTotalQty += dtTable.Rows(i)("Qty")
			grandTotalSale += dtTable.Rows(i)("SalePrice")
			...
			POSTotalQty +=  dtTable.Rows(i)("Qty")
			POSTotalSale += dtTable.Rows(i)("SalePrice")
			POSRetailPrice += dtTable.Rows(i)("TotalRetailPrice")
			POSDiscount += dtTable.Rows(i)("TotalRetailPrice") - dtTable.Rows(i)("SalePrice")
			...
			outputString = outputString.Append("<td align=""left"" class=""" + TextClass + """>" + dtTable.Rows(i)("ProductBarCode") + "</td>")
			outputString = outputString.Append("<td align=""left"" class=""" + TextClass + """>" + dtTable.Rows(i)("ProductName") + "</td>")
			outputString = outputString.Append("<td align=""right"" class=""" + TextClass + """>" + Format(dtTable.Rows(i)("Qty"), FormatData.Rows(0)("QtyFormat"))+ "</td>")
			outputString = outputString.Append("<td align=""right"" class=""" + TextClass + """>" + Format(dtTable.Rows(i)("TotalRetailPrice"), FormatData.Rows(0)("CurrencyFormat")) + "</td>")
			outputString = outputString.Append("<td align=""right"" class=""" + TextClass + """>" + Format(dtTable.Rows(i)("TotalRetailPrice")-dtTable.Rows(i)("SalePrice"), FormatData.Rows(0)("CurrencyFormat")) + "</td>")
			outputString = outputString.Append("<td align=""right"" class=""" + TextClass + """>" + Format(dtTable.Rows(i)("SalePrice"), FormatData.Rows(0)("CurrencyFormat")) + "</td>")

			outputString = outputString.Append("</tr>")

			myDataRow = myDataTable.NewRow()
			myDataRow("Terminal") = HText
			myDataRow("Date") = dtTable.Rows(i)("ERPDate")
			myDataRow("SKU") = dtTable.Rows(i)("ProductCode")
			myDataRow("Qty") = Format(dtTable.Rows(i)("Qty"), FormatData.Rows(0)("QtyFormat"))
			myDataTable.Rows.Add(myDataRow)
			
		Next
		outputString = outputString.Append("<tr bgColor=""" + "white" + """>")
		outputString = outputString.Append("<td align=""right"" class=""" + TextClass + """ colspan=""6"">" + "Grand Total" + "</td>")
		outputString = outputString.Append("<td align=""right"" class=""" + TextClass + """>" + Format(grandTotalQty, FormatData.Rows(0)("QtyFormat")) + "</td>")
		...
		outputString = outputString.Append("<td align=""right"" class=""" + TextClass + """>" + Format(grandTotalSale, FormatData.Rows(0)("CurrencyFormat")) + "</td>")

		outputString = outputString.Append("</tr>")

		ResultText.InnerHtml = outputString.ToString
		
		Session("ERPStockData") = myDataTable
		
		
		Session("ReportResult") = "<table width=""100%""><tr><td align=""center"">" + ResultSearchText.InnerHtml + "</td></tr><tr><td>" + "<table border=""1"" cellpadding=""4"" cellspacing=""0"" style=""border-collapse:collapse;"" width=""100%""><tr>" + TableHeaderText.InnerHtml + "</tr>" + ResultText.InnerHtml + "</table></td></tr></table>"
...
Sub ExportData(Source As Object, E As EventArgs)
	...
	Dim FileName As String = "SaleProductByTerminalData_" & DateTime.Now.ToString("yyyyMMdd_HHmmss", InvC) & ".xls"
...
Sub ExportToERP(Source As Object, E As EventArgs)
	...
	Dim i As Integer
	If Not Session("ERPStockData") Is Nothing Then
		dtTable = Session("ERPStockData")
		Dim filename As String = "StockDataCollection.csv"
		Response.Clear()
		...
		Response.Flush()
		Response.Write("Terminal,Date,SKU,Quantity" + chr(13) & chr(10))
		For i = 0 To dtTable.Rows.Count - 1
			Response.Write(dtTable.Rows(i)("Terminal") + "," + dtTable.Rows(i)("Date") + "," + dtTable.Rows(i)("SKU") + "," + dtTable.Rows(i)("Qty") + chr(13) & chr(10))
		Next
		Response.End()
	End If
...
End Sub
   ...
   Public Function SaleProductTerminal(ByRef PayTypeList As DataTable, ByRef PayTypeData As DataTable, ByVal StartDate As String, ByVal EndDate As String, ByVal ShopID As Integer, ByVal LangID As Integer, ByVal ViewOption As Integer, ByVal objCnn As MySqlConnection) As DataTable
        ...
        sqlStatement = "select a.CloseComputerID As ComputerID,a.SaleDate,DATE_FORMAT(a.SaleDate,'%Y-%m-%d') As ERPDate,dt.DocumentTypeHeader,p.ProductCode,p.ProductBarCode,p.ProductName,SUM(b.Amount) As Qty,SUM(c.TotalRetailPrice) As TotalRetailPrice,SUM(c.SalePrice) As SalePrice from ordertransaction a left outer join orderdetail b ON a.TransactionID=b.TransactionID AND a.ComputerID=b.ComputerID left outer join orderdiscountdetail c ON b.OrderDetailID=c.OrderDetailID AND b.TransactionID=c.TransactionID AND b.ComputerID=c.ComputerID left outer join ComputerName t ON a.CloseComputerID=t.ComputerID left outer join DocumentType dt ON a.DocType=dt.DocumentTypeID AND a.CloseComputerID=dt.ComputerID  left outer join Products p ON b.ProductID=p.ProductID where a.TransactionStatusID=2 AND a.ReceiptID > 0 AND b.ProductSetType NOT IN (-1,-3,14,16) AND a.DocType IN (4,8) AND dt.LangID=1 " + AdditionalQuery + " group by a.CloseComputerID,a.SaleDate,dt.DocumentTypeHeader,p.ProductCode,p.ProductBarCode,p.ProductName order by a.CloseComputerID,a.SaleDate,p.ProductCode"
