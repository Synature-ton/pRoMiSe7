...
<html>
<head>
<title>Sales Report By Payment Type</title>
...
<tr id="ShowShop" runat="server">
	<td valign="top">
		<table>
            ...
            <tr>
				<td><span id="PayTypeList" runat="server"></span></td>
			</tr>
...
Sub Page_Load()
	If User.Identity.IsAuthenticated  AND Session("Report_PaymentType2") Then
	...
	Try	
		...
		Dim PayTypeIDValue As Integer = 0
		
		If Not Request.Form("PayTypeID") Is Nothing Then
			If IsNumeric(Request.Form("PayTypeID")) Then
				PayTypeIDValue = Request.Form("PayTypeID")
			End If
		End If
		
		Dim PayType As DataTable = objDB.List("select * from PayType where Deleted=0 order by PayTypeOrdering", objCnn)
		
		Dim PayTypeString As String = ""
		PayTypeString = "<select name=""PayTypeID"">"
		PayTypeString += "<option value=""" & "0" & """>" & " -- All Pay Type -- "
		For i = 0 To PayType.Rows.Count - 1
			If PayTypeIDValue = PayType.Rows(i)("TypeID") Then
				FormSelected = "selected"
			Else
				FormSelected = ""
			End If
			PayTypeString += "<option value=""" & PayType.Rows(i)("TypeID").ToString & """ " & FormSelected & ">" & PayType.Rows(i)("PayType")
		Next
		PayTypeString += "</select>"
		PayTypeList.InnerHtml = PayTypeString		
		Catch ex As Exception
...
Sub DoSearch(Source As Object, E As EventArgs)
	...
	If FoundError = False Then
		...
		'Application.Lock()
		Dim dtTable As DataTable = PaymentTypeReport(ColumnData,PayTypeList,PayTypeData,ViewOption, StartDate, EndDate, Request.Form("ShopID"), 0, 0, Session("LangID"), Request.Form("PayTypeID"), objCnn)
		...
		HeaderString += "<td align=""center"" class=""smallTdHeader"" bgcolor=""" + GlobalParam.AdminBGColor + """>Income</td>"
		...
		Dim outputResult As String = GenPaymentTypeReport(GlobalParam.GrayBGColor,GlobalParam.AdminBGColor,Request.Form("ShopID"),ViewOption, StartDate, EndDate,  dtTable, Session("LangID"), objCnn)
...
Public Function PaymentTypeReport(ByRef ColumnData As DataTable, ByRef PayTypeList As DataTable, ByRef PayTypeData As DataTable, ByVal ViewOption As Integer, ByVal StartDate As String, ByVal EndDate As String, ByVal ShopID As Integer, ByVal TransactionID As Integer, ByVal ComputerID As Integer, ByVal LangID As Integer, ByVal PayTypeID As Integer, ByVal objCnn As MySqlConnection) As DataTable
		...
		If PayTypeID > 0 Then
            AdditionalQuery += " AND (b.PayTypeID IN (" + PayTypeID.ToString + ") OR b.SmartcardType IN (" + PayTypeID.ToString + "))"
        End If
        ...
        If ViewOption = 2 Then
            OrderBy = " a.SaleDate,a.ReceiptYear,a.ReceiptMonth,a.ReceiptID"
        End If
        ...
        sqlStatement = "select a.*,IF(s.StaffID is NULL,0,s.StaffID) As StaffID,IF(b.SmartCardType>0,c1.TypeID,c.TypeID) As PayTypeID,IF(b.SmartCardType>0,c1.PayType,c.PayType) AS PayTypeName,IF(b.SmartCardType > 0,b.SmartCardType,b.PayTypeID) As CreditCardType,IF(s.StaffFirstName is NULL,'',CONCAT(s.StaffFirstName,' ',s.StaffLastName)) As StaffFullName,IF(s.StaffCode is NULL,'',s.StaffCode) As StaffCode,dt.DocumentTypeHeader,b.Amount As TotalPay,b.CreditCardNo,b.ExpireMonth,b.ExpireYear,sm.SaleModeName,sm.PrefixText from OrderTransaction a inner join PayDetail b ON a.TransactionID=b.TransactionID AND a.ComputerID=b.ComputerID left outer join Staffs s ON a.PaidStaffID=s.StaffID left outer join PayType c ON b.PayTypeID=c.TypeID left outer join PayType c1 ON b.SmartCardType=c1.TypeID left outer join creditcardtype d ON b.CreditCardType=d.CCTypeID left outer join DocumentType dt ON a.DocType=dt.DocumentTypeID AND (a.CloseComputerID=dt.ComputerID OR a.ShopID=dt.ShopID) left outer join SaleMode sm ON a.SaleMode=sm.SaleModeID where dt.LangID=1 " + AdditionalQuery + " order by b.PayTypeID,b.SmartCardType,d.CreditCardType,a.ReceiptYear,a.ReceiptMonth,a.ReceiptID"
	...
	Public Function GenPaymentTypeReport(ByVal GrayBGColor As String, ByVal AdminBGColor As String, ByVal ShopID As Integer, ByVal ViewOption As Integer, ByVal StartDate As String, ByVal EndDate As String, ByVal dtTable As DataTable, ByVal LangID As Integer, ByVal objCnn As MySqlConnection) As String
        ...
        Dim ShopInfo As DataTable
        If ShopID > 0 Then
            ShopInfo = objDB.List("select * from ProductLevel where ProductLevelID=" + ShopID.ToString, objCnn)
        Else
            ShopInfo = objDB.List("select * from ProductLevel where IsShop=1 AND Deleted=0", objCnn)
        End If
        Dim DisplayVATType As String = "V"
		...
		Dim PayTypeName As String
        Dim CurrencyFormatGuest, CurrencyFormatSaleAmount, CurrencyFormatDiscount, CurrencyFormatSale, CurrencyFormatSC, CurrencyFormat, CurrencyFormatPayment, CurrencyFormatIncome, subTotalVAT As Double
        Dim grandTotalGuest, grandTotalSaleAmount, grandTotalDiscount, grandTotalSale, grandTotalSC, grandTotal, grandTotalPayment, grandTotalIncome, grandTotalVAT As Double
        ...
        For i = 0 To dtTable.Rows.Count - 1
			...
			SMText = ""
			If dtTable.Rows(i)("SaleMode") > 1 Then
				SMText = " " + dtTable.Rows(i)("PrefixText")
			End If
            outputString = outputString.Append("<tr>")
			
			PayTypeName = ""
            If i = 0 Then
                PayTypeName = dtTable.Rows(i)("PayTypeName")
            Else
                If dtTable.Rows(i)("PayTypeName") <> dtTable.Rows(i-1)("PayTypeName") Then
					PayTypeName = dtTable.Rows(i)("PayTypeName")
				End If
            End If
			
			outputString = outputString.Append("<td align=""left"" class=""" + TextClass + """>" + PayTypeName + "</td>")
			outputString = outputString.Append("<td align=""left"" class=""" + TextClass + """>" + DisplayString + SMText + "</td>")
            
			outputString = outputString.Append("<td align=""center"" class=""" + TextClass + """>" + DateTimeUtil.FormatDateTime(dtTable.Rows(i)("SaleDate"), "DateOnly", Session(LangID), objCnn) + "</td>")
            outputString = outputString.Append("<td align=""center"" class=""" + TextClass + """>" + DateTimeUtil.FormatDateTime(dtTable.Rows(i)("PaidTime"), "TimeOnly", Session(LangID), objCnn) + "</td>")
            outputString = outputString.Append("<td align=""center"" class=""" + TextClass + """>" + CDbl(dtTable.Rows(i)("NoCustomer")).ToString(FormatObject.QtyFormat, ci) + "</td>")
            outputString = outputString.Append("<td align=""right"" class=""" + TextClass + """>" + CDbl(dtTable.Rows(i)("ReceiptProductRetailPrice")).ToString(FormatObject.CurrencyFormat, ci) + "</td>")
            outputString = outputString.Append("<td align=""right"" class=""" + TextClass + """>" + CDbl(dtTable.Rows(i)("ReceiptDiscount")).ToString(FormatObject.CurrencyFormat, ci) + "</td>")

            outputString = outputString.Append("<td align=""right"" class=""" + TextClass + """>" + CDbl(dtTable.Rows(i)("ReceiptProductRetailPrice")-dtTable.Rows(i)("ReceiptDiscount")).ToString(FormatObject.CurrencyFormat, ci) + "</td>")
			...
			If DisplayVATType = "V" Then
				...
				outputString = outputString.Append("<td align=""right"" class=""" + TextClass + """>" + CDbl(dtTable.Rows(i)("OtherIncome")+dtTable.Rows(i)("OtherIncomeVAT")).ToString(FormatObject.CurrencyFormat, ci) + "</td>")
				 CurrencyFormatSC += dtTable.Rows(i)("ServiceCharge")+dtTable.Rows(i)("ServiceChargeVAT")
				 grandTotalSC += dtTable.Rows(i)("ServiceCharge")+dtTable.Rows(i)("ServiceChargeVAT")
				 CurrencyFormatIncome += dtTable.Rows(i)("OtherIncome")+dtTable.Rows(i)("OtherIncomeVAT")
				 grandTotalIncome += dtTable.Rows(i)("OtherIncome")+dtTable.Rows(i)("OtherIncomeVAT")
			Else
				...
				outputString = outputString.Append("<td align=""right"" class=""" + TextClass + """>" + CDbl(dtTable.Rows(i)("OtherIncome")).ToString(FormatObject.CurrencyFormat, ci) + "</td>")
				...
				CurrencyFormatSC += dtTable.Rows(i)("ServiceCharge")
				grandTotalSC += dtTable.Rows(i)("ServiceCharge")
				CurrencyFormatIncome += dtTable.Rows(i)("OtherIncome")
				grandTotalIncome += dtTable.Rows(i)("OtherIncome")
			End If
            outputString = outputString.Append("<td align=""right"" class=""" + TextClass + """>" + CDbl(dtTable.Rows(i)("ReceiptSalePrice")).ToString(FormatObject.CurrencyFormat, ci) + "</td>")
            ...
            CurrencyFormatGuest += dtTable.Rows(i)("NoCustomer")
            CurrencyFormatSaleAmount += dtTable.Rows(i)("ReceiptProductRetailPrice")
            CurrencyFormatDiscount += dtTable.Rows(i)("ReceiptDiscount")
            CurrencyFormatSale += dtTable.Rows(i)("ReceiptProductRetailPrice") - dtTable.Rows(i)("ReceiptDiscount")
           
            CurrencyFormat += dtTable.Rows(i)("ReceiptSalePrice")
            CurrencyFormatPayment += dtTable.Rows(i)("TotalPay")
			subTotalVAT += dtTable.Rows(i)("TransactionVAT")

            'StaffTotalGuest += dtTable.Rows(i)("NoCustomer")
            'StaffTotalSaleAmount += dtTable.Rows(i)("ReceiptProductRetailPrice")
            ...
            grandTotalSaleAmount += dtTable.Rows(i)("ReceiptProductRetailPrice")
            grandTotalDiscount += dtTable.Rows(i)("ReceiptDiscount")
            grandTotalSale += dtTable.Rows(i)("ReceiptProductRetailPrice") - dtTable.Rows(i)("ReceiptDiscount")
            
            grandTotal += dtTable.Rows(i)("ReceiptSalePrice")
            ...
            If i <= dtTable.Rows.Count - 1 Then
                If i = dtTable.Rows.Count - 1 Then
                    ...
                    outputString = outputString.Append("<td align=""left"" class=""" + "smallboldtext" + """>" + "Total " + dtTable.Rows(i)("PayTypeName") + "</td>")
                    outputString = outputString.Append("<td align=""left"" class=""" + "smallboldtext" + """>" + "" + "</td>")
                    outputString = outputString.Append("<td align=""left"" class=""" + "smallboldtext" + """>" + "" + "</td>")
					outputString = outputString.Append("<td align=""left"" class=""" + "smallboldtext" + """>" + "" + "</td>")
					outputString = outputString.Append("<td align=""left"" class=""" + "smallboldtext" + """>" + "" + "</td>")
					...
					outputString = outputString.Append("<td align=""left"" class=""" + "smallboldtext" + """>" + "" + "</td>")
					outputString = outputString.Append("<td align=""left"" class=""" + "smallboldtext" + """>" + "" + "</td>")
                    'outputString = outputString.Append("<td align=""center"" class=""" + "smallboldtext" + """>" + CDbl(CurrencyFormatGuest).ToString(FormatObject.CurrencyFormat, ci) + "</td>")
                    'outputString = outputString.Append("<td align=""right"" class=""" + "smallboldtext" + """>" + CDbl(CurrencyFormatSaleAmount).ToString(FormatObject.CurrencyFormat, ci) + "</td>")
                    'outputString = outputString.Append("<td align=""right"" class=""" + "smallboldtext" + """>" + CDbl(CurrencyFormatDiscount).ToString(FormatObject.CurrencyFormat, ci) + "</td>")

                    'outputString = outputString.Append("<td align=""right"" class=""" + "smallboldtext" + """>" + CDbl(CurrencyFormatSale).ToString(FormatObject.CurrencyFormat, ci) + "</td>")
                    'outputString = outputString.Append("<td align=""right"" class=""" + "smallboldtext" + """>" + CDbl(CurrencyFormatSC).ToString(FormatObject.CurrencyFormat, ci) + "</td>")
					'outputString = outputString.Append("<td align=""right"" class=""" + "smallboldtext" + """>" + CDbl(CurrencyFormatIncome).ToString(FormatObject.CurrencyFormat, ci) + "</td>")
					If DisplayVATType = "E" Then
					'		outputString = outputString.Append("<td align=""right"" class=""" + "smallboldtext" + """>" + CDbl(subTotalVAT).ToString(FormatObject.CurrencyFormat, ci) + "</td>")
					End If
                    'outputString = outputString.Append("<td align=""right"" class=""" + "smallboldtext" + """>" + CDbl(CurrencyFormat).ToString(FormatObject.CurrencyFormat, ci) + "</td>")
                    outputString = outputString.Append("<td align=""right"" class=""" + "smallboldtext" + """>" + CDbl(CurrencyFormatPayment).ToString(FormatObject.CurrencyFormat, ci) + "</td>")
                    outputString = outputString.Append("<td align=""left"" class=""" + "smallboldtext" + """>" + "" + "</td>")
                    outputString = outputString.Append("</tr>")
					...
					outputString = outputString.Append("<td align=""left"" class=""" + "smallboldtext" + """>" + "" + "</td>")
					outputString = outputString.Append("<td align=""left"" class=""" + "smallboldtext" + """>" + "" + "</td>")
                    'outputString = outputString.Append("<td align=""center"" class=""" + "smallboldtext" + """>" + CDbl(grandTotalGuest).ToString(FormatObject.CurrencyFormat, ci) + "</td>")
                    'outputString = outputString.Append("<td align=""right"" class=""" + "smallboldtext" + """>" + CDbl(grandTotalSaleAmount).ToString(FormatObject.CurrencyFormat, ci) + "</td>")
                    'outputString = outputString.Append("<td align=""right"" class=""" + "smallboldtext" + """>" + CDbl(grandTotalDiscount).ToString(FormatObject.CurrencyFormat, ci) + "</td>")

                    'outputString = outputString.Append("<td align=""right"" class=""" + "smallboldtext" + """>" + CDbl(grandTotalSale).ToString(FormatObject.CurrencyFormat, ci) + "</td>")
                    'outputString = outputString.Append("<td align=""right"" class=""" + "smallboldtext" + """>" + CDbl(grandTotalSC).ToString(FormatObject.CurrencyFormat, ci) + "</td>")
					'outputString = outputString.Append("<td align=""right"" class=""" + "smallboldtext" + """>" + CDbl(grandTotalIncome).ToString(FormatObject.CurrencyFormat, ci) + "</td>")
					If DisplayVATType = "E" Then
					'		outputString = outputString.Append("<td align=""right"" class=""" + "smallboldtext" + """>" + CDbl(grandTotalVAT).ToString(FormatObject.CurrencyFormat, ci) + "</td>")
					End If
                    'outputString = outputString.Append("<td align=""right"" class=""" + "smallboldtext" + """>" + CDbl(grandTotal).ToString(FormatObject.CurrencyFormat, ci) + "</td>")
                    outputString = outputString.Append("<td align=""right"" class=""" + "smallboldtext" + """>" + CDbl(grandTotalPayment).ToString(FormatObject.CurrencyFormat, ci) + "</td>")
                ...
                Else
                    If dtTable.Rows(i)("CreditCardType") <> dtTable.Rows(i + 1)("CreditCardType") Then
                        ...
                        outputString = outputString.Append("<td align=""left"" class=""" + "smallboldtext" + """>" + "" + "</td>")
                        outputString = outputString.Append("</tr>")
                        CurrencyFormatGuest = 0
                        CurrencyFormatSaleAmount = 0
                        CurrencyFormatDiscount = 0
                        CurrencyFormatSale = 0
                        CurrencyFormatSC = 0
                        CurrencyFormat = 0
                        CurrencyFormatPayment = 0
						subTotalVAT = 0
						CurrencyFormatIncome = 0
                    End If
